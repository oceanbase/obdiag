...
|4.1.0|2026.02| 2026.02.11 |<ul><li> 新增 `obdiag display scene run --scene=observer.log_volume_statistics` 命令，CLOG 日志量/容量统计</li><li> 新增 `obdiag display-trace` 命令，查看单次执行的详细日志</li><li> `obdiag gather perf` 支持生成火焰图 SVG</li><li> `obdiag analyze log` 新增 `--tenant_id` 参数，`obdiag analyze flt_trace` 支持按客户端 IP 过滤</li><li> `obdiag gather plan_monitor` 报告增加统计信息直方图</li><li> `obdiag check run` 和 `obdiag rca run` 支持 HTML 报告格式</li><li> 检查框架由 checker 重构为 check（TaskBase、Python 巡检任务）</li></ul>|
|4.2.0|2026.03| 2026.03.13 |<ul><li> 检查框架重构</li><li> 支持收集 obproxy 进程的 perf 信息</li><li> GatherPlanMonitor 解析表去重</li><li> 优化 check run 的 SSH 连接处理</li><li> 巡检场景扩展 </li></ul>|
