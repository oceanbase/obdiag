...
class TaskBase(object):
    ...
    def execute_one_node(self, steps_nu, node):
        try:
            self.stdio.verbose(
                "run task in node: {0}".format(
                    StringUtils.node_cut_passwd_for_log(node)
                )
            )
            ...
            for step in steps["steps"]:
                try:
                    ...
                    step_run = StepBase(
                        self.context, step, node, self.cluster, task_variable_dict
                    )
                    ...
                    if (
                        "report_type" in step["result"]
                        and step["result"]["report_type"] == "execution"
                    ):
                        self.stdio.verbose("report_type stop this step")
                        return
                except StepExecuteFailException as e:
                    self.stdio.error(
                        "TaskBase execute CheckStepFailException: {0} . Do Next Task".format(
                            e
                        )
                    )
                    return
                    ...
                    self.stdio.warn(
                        "TaskBase execute StepResultFalseException: {0} .".format(e)
                    )
                    continue
                except StepResultFailException as e:
                    self.stdio.warn(
                        "TaskBase execute StepResultFailException: {0}".format(e)
                    )
                    return
