...
class DataSizeHandler:
    def __init__(self, context, step, node, task_variable_dict):
        ...
        try:
            ...
            self.stdio.error(
                "DataSizeHandler ssh init fail  . Please check the NODES conf Exception : {0} .".format(
                    e
                )
            )
            raise Exception(
        ...
        try:
            if "key" not in self.step:
                raise StepExecuteFailException(
                    "DataSizeHandler execute parameter's 'key' is not set"
                )
            ...
            self.stdio.verbose(
                "DataSizeHandler set {0} = {1}".format(s, self.task_variable_dict[s])
            )
        except Exception as e:
            self.stdio.error("DataSizeHandler execute Exception: {0}".format(e).strip())
            raise StepExecuteFailException(
                "DataSizeHandler execute Exception: {0}".format(e).strip()
            )
