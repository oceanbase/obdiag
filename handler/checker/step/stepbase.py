...
from handler.checker.check_exception import (
    StepResultFailException,
    StepExecuteFailException,
    ResultFalseException,
    ResultFailException,
    StepResultFalseException,
)
        ...
        try:
            ...
            if "ip" in self.node:
                ...
                self.task_variable_dict["remote_ip"] = (
                    docker.from_env()
                    .containers.get(self.node["container_name"])
            ...
            if self.step["type"] == "get_system_parameter":
                handler = GetSystemParameterHandler(
                    self.context, self.step, self.node, self.task_variable_dict
                )
                ...
                handler = StepSQLHandler(
                    self.context, self.step, task_variable_dict=self.task_variable_dict
                )
            elif self.step["type"] == "data_size":
                handler = DataSizeHandler(
                    self.context, self.step, self.cluster, self.task_variable_dict
                )
            else:
                raise StepExecuteFailException(
                    "the type not support: {0}".format(self.step["type"])
                )
            ...
            if self.step["type"] == "sql":
                report.add(
                    "[cluster:{0}] {1}".format(
                        self.cluster.get("ob_cluster_name")
                        or self.cluster.get("obproxy_cluster_name")
                        or no_cluster_name_msg,
                        e,
                    ),
                    "fail",
                )
                ...
                report.add(
                    "[{0}:{1}] {2}".format(
                        self.node.get("ssh_type") or "",
                        self.node.get("container_name")
                        or self.task_variable_dict.get("remote_ip")
                        or "",
                        e,
                    ),
                    "fail",
            ...
            raise StepExecuteFailException(
                "StepBase handler.execute fail {0}".format(e)
            )

        try:
            self.task_variable_dict = handler.update_step_variable_dict()
            self.stdio.verbose(
                "self.task_variable_dict: {0}".format(self.task_variable_dict)
            )
            if (
                self.step["type"] == "get_system_parameter"
                and "result" in self.step
                and "set_value" in self.step["result"]
                and self.task_variable_dict[self.step["result"]["set_value"]] == ""
            ):
                return
            ...
            if "result" in self.step:
                ...
                result = CheckResult(
                    self.context, self.step["result"], self.task_variable_dict
                )
                result.execute()
                if (
                    "report_type" in self.step["result"]
                    and self.step["result"]["report_type"] == "execution"
                ):
            ...
            self.stdio.warn(
                "step_base ResultFalseException:{0}".format(resultException)
            )
            level = "critical"
            self.stdio.verbose(
                "step_base ResultFalseException self.step.result:{0}".format(
                    self.step["result"]
                )
            )
            if "result" in self.step:
                if "report_type" in self.step["result"]:
                    self.stdio.verbose(
                        "report_type use is  {0}".format(
                            self.step["result"]["report_type"]
                        )
                    )
            ...
            self.stdio.error(
                "step_base ResultFailException:{0}".format(resultFailException)
            )
            if self.step["type"] == "sql":
                report.add(
                    "[cluster:{0}] {1}".format(
