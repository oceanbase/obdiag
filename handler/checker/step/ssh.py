...
class SshHandler:
    ...
    def execute(self):
        try:
            ...
            ssh_cmd = StringUtils.build_str_on_expr_by_dict(
            ...
            self.stdio.verbose(
                "ssh result:{0}".format(Util.convert_to_number(ssh_report_value))
            )
            if "result" in self.step and "set_value" in self.step["result"]:
                self.stdio.verbose(
                    "ssh result set {0}".format(
                        self.step["result"]["set_value"],
            ...
            raise StepExecuteFailException(
                "ssh execute Exception:{0}".format(e).strip()
            )
        finally:
            self.ssh_helper.ssh_close()
        self.stdio.verbose(
            "step SshHandler ssh_report_value:{0}".format(ssh_report_value)
        )
