...
class CheckHandler:
    ...
    def __init__(self, context, check_target_type="observer"):
        ...
        self.export_report_type = (
            self.context.inner_config["check"]["report"]["export_type"] or "table"
        )
        self.ignore_version = (
            self.context.inner_config["check"]["ignore_version"] or False
        )
        ...
        if check_target_type is not None:
            case_package_file = (
                self.work_path + "/" + check_target_type + "_check_package.yaml"
            )
        else:
        ...
        if os.path.exists(case_package_file):
            ...
            raise CheckException(
                "case_package_file {0} is not exist".format(case_package_file)
            )
        ...
        if os.path.exists(tasks_base_path):
            ...
            raise CheckException(
                "tasks_base_path {0} is not exist".format(tasks_base_path)
            )
        ...
        for node in self.nodes:
            ...
            try:
                ...
                self.stdio.warn(
                    "StepBase get SshHelper fail on{0} ,Exception: {1}".format(
                        node.get("ip"), e
                    )
                )
            node["ssher"] = ssher
        ...
        self.version = get_version(
            self.nodes, self.check_target_type, self.cluster, self.stdio
        )
    ...
    def handle(self):
        try:
            package_name = None
            if self.check_target_type == "obproxy" and Util.get_option(
                self.options, "obproxy_cases"
            ):
                package_name = Util.get_option(self.options, "obproxy_cases")

            if self.check_target_type == "observer" and Util.get_option(
                self.options, "cases"
            ):
    ...
    def get_package_tasks(self, package_name):
        ...
        self.stdio.verbose(
            "by cases name: {0} , get cases: {1}".format(
                package_name, packege_tasks[package_name]
            )
        )
        ...
        try:
            ...
            if not self.ignore_version:
                ...
                if version:
                    ...
                    self.stdio.verbose(
                        "cluster.version is {0}".format(self.cluster["version"])
                    )
                    task = TaskBase(
                        self.context,
    ...
    def execute(self):
        try:
            self.stdio.verbose(
                "execute_all_tasks. the number of tasks is {0} ,tasks is {1}".format(
                    len(self.tasks.keys()), self.tasks.keys()
                )
            )
