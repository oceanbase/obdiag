...
class DDlDiskFullScene(RcaScene):
    ...
    def init(self, context):
        ...
        if not (
            observer_version == "4.2.1.0"
            or StringUtils.compare_versions_greater(observer_version, "4.2.1.0")
        ):
            self.stdio.error(
                "observer version is {0}, which is less than 4.2.1.0.".format(
                    observer_version
                )
            )
            raise RCAInitException(
        ...
        if (
            table_name is None
            or table_name == ""
            or tenant_name is None
            or tenant_name == ""
        ):
            raise RCAInitException(
                "table_name or tenant_name is None. Please check the input parameters."
            )
        if action_type is not None:
            if action_type == "add_index":
                ...
                if index_name is not None and index_name.strip() != "":
                    ...
                    self.action_type = None
                    self.stdio.error(
                        "action type is {0}. but index_name is None. Please input it.".format(
                            action_type
                        )
                    )
            else:
                self.stdio.error(
                    "action type is {0}, but only support add_index now.".format(
                        action_type
                    )
                )

        tenant_data = self.ob_connector.execute_sql(
            "select tenant_id from oceanbase.__all_tenant where tenant_name = '{0}';".format(
                tenant_name
            )
        )
        if len(tenant_data) == 0:
            raise RCAInitException(
                "can not find tenant id by tenant name: {0}. Please check the tenant name.".format(
                    tenant_name
                )
            )
        ...
        table_id_data = self.ob_connector.execute_sql(
            "select table_id from oceanbase.__all_virtual_table where table_name = '{0}';".format(
                table_name
            )
        )
        if len(table_id_data) == 0:
            raise RCAInitException(
                "can not find table id by table name: {0}. Please check the table name.".format(
                    table_name
                )
            )
        ...
        self.verbose(
            "table_id is {0}, tenant_id is {1}.".format(self.table_id, self.tenant_id)
        )
        ...
        try:
            ...
            if len(tablet_size_data) <= 0 or tablet_size_data[0][2] is None:
                raise RCAExecuteException(
                    "can not find tablet size info or estimated_data_size. please check the data:{0}.".format(
                        tablet_size_data
                    )
                )
            ...
            if self.action_type is not None and self.action_type == "add_index":
                ...
                sql = "select table_id from oceanbase.__all_virtual_table_history where tenant_id = '{0}' and data_table_id = '{1}' and table_name like '%{2}%';".format(
                    self.tenant_id, self.table_id, self.index_name
                )
                self.verbose("execute_sql is {0}".format(sql))
                ...
                sql = "select table_id, sum(data_length) from oceanbase.__all_virtual_column_history where tenant_id = '{0}' and table_id = '{1}';".format(
                    self.tenant_id, self.table_id
                )
                self.verbose("execute_sql is {0}".format(sql))
                main_table_sum_of_data_length = int(
                    self.ob_connector.execute_sql(sql)[0][1]
                )
                self.verbose(
                    "main_table_sum_of_data_length is {0}".format(
                        main_table_sum_of_data_length
                    )
                )
                record.add_record(
                    "main_table_sum_of_data_length is {0}".format(
                ...
                sql = "select table_id, sum(data_length) from oceanbase.__all_virtual_column_history where tenant_id = '{0}' and table_id = '{1}';".format(
                    self.tenant_id, self.index_table_id
                )
                self.verbose("execute_sql is {0}".format(sql))
                index_table_sum_of_data_length = int(
                    self.ob_connector.execute_sql(sql)[0][1]
                )
                self.verbose(
                    "index_table_sum_of_data_length is {0}".format(
                        index_table_sum_of_data_length
                    )
                )
                record.add_record(
                    "index_table_sum_of_data_length is {0}".format(
                ...
                for node_estimated_size in self.estimated_size:
                    new_node_estimated_size = [
                        node_estimated_size[0],
                        node_estimated_size[1],
                    ]
                    estimiated_index_size = int(
                        index_table_sum_of_data_length
                        / main_table_sum_of_data_length
                        / 1024
                        / 1024
                        * int(node_estimated_size[2])
                    )
            ...
            for estimated_size in self.estimated_size:
                ...
                self.verbose(
                    "On target_server_ip is {0}, target_server_port is {1}, target_server_estimated_size is {2}".format(
                        target_server_ip,
                        target_server_port,
                        target_server_estimated_size,
                    )
                )
                record.add_record(
                    "On target_server_ip is {0}, target_server_port is {1}, target_server_estimated_size is {2}".format(
                ...
                target_server_data = self.ob_connector.execute_sql(
                    "select total_size, used_size from oceanbase.__all_virtual_disk_stat where svr_ip = '{0}' and svr_port = {1};".format(
                        target_server_ip, target_server_port
                    )
                )
                target_server_total_size = int(target_server_data[0][0])
                self.verbose(
                    "target_server_total_size is {0}".format(target_server_total_size)
                )
                record.add_record(
                    "target_server_total_size is {0}".format(target_server_total_size)
                )
                ...
                self.verbose(
                    "target_server_used_size is {0}".format(target_server_used_size)
                )
                record.add_record(
                    "target_server_used_size is {0}".format(target_server_used_size)
                )
                ...
                sql = "SELECT VALUE FROM oceanbase.GV$OB_PARAMETERS WHERE SVR_IP='{0}' and SVR_PORT='{1}' and NAME LIKE  \"data_disk_usage_limit_percentage\"".format(
                    target_server_ip, target_server_port
                )
                self.verbose("execute_sql is {0}".format(sql))
                data_disk_usage_limit_percentage = int(
                    self.ob_connector.execute_sql(sql)[0][0]
                )
                # data_disk_usage_limit_percentage is a Cluster level configuration items
                self.verbose(
                    "data_disk_usage_limit_percentage is {0}".format(
                ...
                if (
                    self.observer_version == "4.3.0.0"
                    or StringUtils.compare_versions_greater(
                        self.observer_version, "4.3.0.0"
                    )
                ):
                    target_server_estimated_size = int(
                        target_server_estimated_size * 15 / 10
                    )
                else:
                    target_server_estimated_size = int(
                        target_server_estimated_size * 55 / 10
                    )
                self.verbose(
                    "target_server_estimated_size is {0}".format(
                        target_server_estimated_size
                    )
                )
                record.add_record(
                ...
                available_disk_space = int(
                    target_server_total_size / 100 * data_disk_usage_limit_percentage
                    - target_server_used_size
                )
                self.verbose("available_disk_space is {0}".format(available_disk_space))
                record.add_record(
                    "available_disk_space is {0}".format(available_disk_space)
                )

                if target_server_estimated_size - available_disk_space > 0:
                    record.add_record(
                        "target_server_estimated_size - available_disk_space is {0}".format(
                            target_server_estimated_size - available_disk_space
                        )
                    )
                    record.add_suggest(
                        "the disk space of server({0}:{1}) disk is not enough.  please add the server disk".format(
                            target_server_ip, target_server_port
                        )
                    )
                else:
                    ...
                    record.add_suggest(
                        "the disk space of server({0}:{1}) is enough. Don't warn ".format(
                            target_server_ip, target_server_port
                        )
                    )
