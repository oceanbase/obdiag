        ...
        try:
            ...
            for tenant_id_data in tenant_ids_data:
                ...
                for ls_data in ls_datas:
                    tenant_ls_datas.append({
                        "tenant_id":tenant_id,
                        "ls_id":ls_data["ls_id"],
                        "ip":ls_data["svr_ip"],
                        "port":ls_data["svr_port"]
                    })
            ...
            for tenant_ls_data in tenant_ls_datas:
                ...
                if  record.suggest_is_empty():
                     record.add_suggest(
                        "not found stuck about clog disk full. You can package the files under '{0}' and upload them to the OceanBase community for further analysis.".format(
                            self.work_path))
