...
class RCAHandler:
    def __init__(self, context):
        ...
        try:
            ...
            self.stdio.warn(
                "RCAHandler init ob_connector failed: {0}. If the scene need it, please check the conf.yaml".format(
                    str(e)
                )
            )
        # build report
        ...
        if self.context.get_variable("obproxy_version", default="") == "":
            if len(obproxy_nodes) > 0:
                ...
                try:
                    ...
                    self.stdio.warn(
                        "RCAHandler.init Failed to get obproxy version. Error:{0}".format(
                            e
                        )
                    )
                if obproxy_version != "":
                    self.stdio.verbose(
                        "RCAHandler.init get obproxy version: {0}".format(
                            obproxy_version
                        )
                    )
                else:
                ...
                self.stdio.verbose(
                    "RCAHandler.init get obproxy version: {0}".format(obproxy_version)
                )
        ...
        if rca_scene_parameters != "":
            try:
                ...
                raise Exception(
                    "Failed to parse input_parameters. Please check the option is json:{0}".format(
    ...
    def handle(self):
        ...
        if scene_name:
            ...
            raise Exception(
                "rca_scene :{0} is not exist or not input".format(scene_name)
            )
...
class RcaScene:
    ...
    def execute(self):
        ...
        raise Exception(
            "rca ({0}) scene.execute() undefined".format(type(self).__name__)
        )

    def get_result(self):
        # 设定场景分析的返回场景使用说明，需要的参数等等
        raise Exception(
            "rca ({0}) scene.get_result() undefined".format(type(self).__name__)
        )

    def get_scene_info(self):
        raise Exception(
            "rca ({0})  scene.get_scene_info() undefined".format(type(self).__name__)
        )
    ...
    def get_all_tenants_id(self):
        try:
            ...
            all_tenant_id_data = self.ob_connector.execute_sql(
                "select tenant_id from oceanbase.__all_tenant;"
            )[0]
...
class Result:
    ...
    def export(self):
        record_file_name = os.path.expanduser(
            "{0}/{1}".format(self.save_path, "record")
        )
