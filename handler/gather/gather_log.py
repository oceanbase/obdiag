...
class GatherLogHandler(BaseShellHandler):
    ...
    def init_option(self):
        ...
        if from_option is not None and to_option is not None:
            ...
            self.stdio.print(
                "gather log from_time: {0}, to_time: {1}".format(
    ...
    def handle(self):
        ...
        last_info = (
            "For result details, please run cmd \033[32m' cat {0} '\033[0m\n".format(
                os.path.join(pack_dir_this_command, "result_summary.txt")
            )
        )
        return True
    ...
    def __handle_from_node(self, pack_dir_this_command, node):
        ...
        if "ssh_type" in node and node["ssh_type"] == "docker":
            local_store_dir = "{0}/docker_{1}".format(
                pack_dir_this_command, node["container_name"]
            )
        else:
        ...
        if not ssh_failed:
            ...
            if is_empty:
                ...
                self.__handle_zip_file(
                    node.get("ip"), ssh, resp, gather_dir_name, pack_dir_this_command
                )
            ssh.ssh_close()
        return resp
    ...
    def __grep_log_until_empty(self, ssh_helper, home_path, log_list, gather_path):
        ...
        other_log_list = [
            log_name
            for log_name in log_list
            if not any(log_name.startswith(prefix) for prefix in log_type_list)
        ]
        ...
        log_list = [
            log_name
            for log_name in log_list
            if (log_name not in other_log_list) and log_name[-1].isdigit()
        ]

        for log_type in log_type_list:
            cur_type_log_list = [
                log_name for log_name in log_list if log_name.startswith(log_type)
            ]
            # 按照时间从最新的到最旧的
    ...
    def __grep_log(self, ssh_helper, home_path, log_name, gather_path):
        ...
        if self.grep_options is not None:
            ...
            find_file_cmd = (
                "find {gather_path} -type f -name {log_name} ! -empty".format(
                    gather_path=gather_path, log_name=log_name
                )
            )
            ...
            find_file_res = (
                SshClient(self.stdio).run(ssh_helper, find_file_cmd)
                if self.is_ssh
                else LocalClient(self.stdio).run(grep_cmd)
            )
    ...
    def __handle_log_list(self, ssh, node, resp):
        ...
        if len(log_list) > self.file_number_limit:
            self.stdio.warn(
                ...
                "Please adjust the query limit".format(
                    ip, len(log_list), self.file_number_limit
                )
            )
            resp["skip"] = (True,)
            resp["error"] = "Too many files {0} > {1}".format(
            ...
            self.stdio.warn(
                ...
                " Please adjust the query time limit.".format(
                    ip, len(log_list), self.from_time_str, self.to_time_str
                )
            )
            resp["skip"] = (True,)
    ...
    def __get_log_name(self, ssh_helper, node):
        ...
        if log_files:
            ...
            self.stdio.error(
                "Unable to find the log file. Please provide the correct home_path, the default is [/root/observer]"
            )
            ...
            if type(self.grep_options) == str:
                ...
                for grep_option in self.grep_options:
                    if type(grep_option) != str:
                        self.stdio.error(
                            "The grep args must be string or list of strings, but got {0}".format(
                                type(grep_option)
                            )
                        )
                        raise Exception(
                        ...
                        self.stdio.warn(
                            'The grep args must be string or list of strings, but got ""'
                        )
                        continue
    ...
    def __handle_zip_file(self, ip, ssh, resp, gather_dir_name, pack_dir_this_command):
        zip_password = ""
        gather_dir_full_path = "{0}/{1}".format(
            self.gather_ob_log_temporary_dir, gather_dir_name
        )
        ...
        if self.zip_encrypt:
            ...
            zip_encrypt_dir(
                ...
                ssh,
                zip_password,
                self.gather_ob_log_temporary_dir,
                gather_dir_name,
                self.stdio,
            )
        else:
        ...
        gather_log_file_size = get_file_size(
            self.is_ssh, ssh, gather_package_dir, self.stdio
        )
        ...
        if int(gather_log_file_size) < self.file_size_limit:
            local_store_path = pack_dir_this_command + "/{0}.zip".format(
                gather_dir_name
            )
            download_file(
                self.is_ssh, ssh, gather_package_dir, local_store_path, self.stdio
            )
            resp["error"] = ""
        ...
        self.stdio.verbose(
            "Collect pack gathered from node {0}: stored in {1}".format(
                ip, gather_package_dir
            )
        )
        return resp
