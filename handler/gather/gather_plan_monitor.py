...
class GatherPlanMonitorHandler(object):
    ...
    def init_option(self):
        ...
        if store_dir_option and store_dir_option != "./":
            if not os.path.exists(os.path.abspath(store_dir_option)):
                self.stdio.warn(
                    "warn: option --store_dir [{0}] incorrect: No such directory, Now create it".format(
    ...
    def __init_db_connector(self):
        self.db_connector = OBConnector(
            ...
            password=self.db_conn.get("password"),
    ...
    def handle(self):
        ...
        if self.is_scene:
            ...
            pack_dir_this_command = os.path.join(
        ...
        self.report_file_path = os.path.join(
            pack_dir_this_command, "sql_plan_monitor_report.html"
        )
        ...
        def handle_plan_monitor_from_ob(cluster_name):
            ...
            st = time.time()
            resp = self.init_resp()
            result_sql_audit_by_trace_id_limit1 = (
                self.select_sql_audit_by_trace_id_limit1()
            )
            if len(result_sql_audit_by_trace_id_limit1) > 0:
                ...
                sql_plan_monitor_svr_agg_template = (
                    self.sql_plan_monitor_svr_agg_template_sql()
                )
                sql_plan_monitor_svr_agg_v1 = (
                    str(sql_plan_monitor_svr_agg_template)
                ...
                sql_plan_monitor_detail_template = (
                    self.sql_plan_monitor_detail_template_sql()
                )
                sql_plan_monitor_detail_v1 = (
                    str(sql_plan_monitor_detail_template)
                    .replace("##REPLACE_TRACE_ID##", trace_id)
                    .replace(
                        "##REPLACE_ORDER_BY##",
                ...
                sql_plan_monitor_detail_v2 = (
                    str(sql_plan_monitor_detail_template)
                    .replace("##REPLACE_TRACE_ID##", trace_id)
                    .replace(
                        "##REPLACE_ORDER_BY##",
                        "PROCESS_NAME ASC, PLAN_LINE_ID ASC, FIRST_REFRESH_TIME ASC",
                    )
                )

                sql_plan_monitor_dfo_op = self.sql_plan_monitor_dfo_op_sql(
                    tenant_id, plan_id, trace_id
                )
                full_audit_sql_by_trace_id_sql = self.full_audit_sql_by_trace_id_sql(
                    trace_id
                )
                plan_explain_sql = self.plan_explain_sql(
                    tenant_id, plan_id, svr_ip, svr_port
                )
                ...
                # 输出sql explain的信息
                self.stdio.verbose(
                    "[sql plan monitor report task] report plan explain, sql: [{0}]".format(
                        sql
                    )
                )
                ...
                # 输出sql_audit的详细信息
                self.stdio.verbose(
                    "[sql plan monitor report task] report sql_audit details"
                )
                self.report_sql_audit_details(full_audit_sql_by_trace_id_sql)
                # 输出算子信息 表+图
                self.stdio.verbose(
                    "[sql plan monitor report task] report sql plan monitor dfo"
                )
                self.report_sql_plan_monitor_dfo_op(sql_plan_monitor_dfo_op)
                # 输出算子信息按 svr 级汇总 表+图
                self.stdio.verbose(
                    "[sql plan monitor report task] report sql plan monitor group by server"
                )
                self.report_sql_plan_monitor_svr_agg(
                    sql_plan_monitor_svr_agg_v1, sql_plan_monitor_svr_agg_v2
                )
                self.report_fast_preview()
                # 输出算子信息按算子维度聚集
                self.stdio.verbose(
                    "[sql plan monitor report task] sql plan monitor detail operator"
                )
                self.report_sql_plan_monitor_detail_operator_priority(
                    sql_plan_monitor_detail_v1
                )
                # 输出算子信息按线程维度聚集
                self.stdio.verbose(
                    "[sql plan monitor report task] sql plan monitor group by priority"
                )
                self.reportsql_plan_monitor_detail_svr_priority(
                    sql_plan_monitor_detail_v2
                )
                ...
                self.__report(
                    "<div class='help' style='font-size:11px'>DFO 级<hr /><pre>%s</pre></div><br/>"
                    % (sql_plan_monitor_dfo_op)
                )
                self.__report(
                    "<div class='help' style='font-size:11px'>机器级<hr /><pre>%s</pre></div><br/>"
                    % (sql_plan_monitor_svr_agg_v1)
                )
                self.__report(
                    "<div class='help' style='font-size:11px'>线程级<hr /><pre>%s</pre></div><br/>"
                    % (sql_plan_monitor_detail_v1)
                )

                t = time.localtime(time.time())
                self.__report(
                    "报告生成时间： %s" % (time.strftime("%Y-%m-%d %H:%M:%S", t))
                )
                self.report_footer()
            ...
            gather_tuples.append(
                (cluster_name, False, "", int(time.time() - st), pack_dir_this_command)
            )
        ...
        # 将汇总结果持久化记录到文件中
        FileUtil.write_append(
            os.path.join(pack_dir_this_command, "result_summary.txt"), summary_tuples
        )
    ...
    def __init_db_conn(self, env):
        try:
            ...
            if StringUtils.validate_db_info(self.db_conn):
                ...
                self.stdio.error(
                    "db connection information requird [db_connect = '-hxx -Pxx -uxx -pxx -Dxx'] but provided {0}, please check the --env {0}".format(
                        env_dict
                    )
                )
                self.db_connector = self.sys_connector
            ...
            self.stdio.exception(
                "init db connector, error: {0}, please check --env option "
            )
    ...
    def __get_overall_summary(node_summary_tuple):
        ...
        for tup in node_summary_tuple:
            ...
            summary_tab.append(
                (
                    cluster,
                    "Error" if is_err else "Completed",
                    "{0} s".format(int(consume_time)),
                    pack_path,
                )
            )
    ...
    def report_schema(self, sql):
        try:
            ...
            if self.enable_dump_db:
                ...
                for t in valid_words:
                    try:
                        ...
                        schemas = (
                            schemas
                            + "<pre style='margin:20px;border:1px solid gray;'>{0}</pre>".format(
                                data[0][1]
                            )
                        )
                        ...
                        pass
            cursor = self.sys_connector.execute_sql_return_cursor(
                "show variables like '%parallel%'"
            )
            s = from_db_cursor(cursor)
            s.align = "l"
            schemas = (
                schemas + "<pre style='margin:20px;border:1px solid gray;'>%s</pre>" % s
            )
            ...
            self.__report(
                "<div><h2 id='schema_anchor'>SCHEMA 信息</h2><div id='schema' style='display: none'>"
                + schemas
                + "</div></div>"
            )
            cursor.close()
    ...
    def init_monitor_stat(self):
        sql = (
            ...
            + ("SYS." if self.tenant_mode == "oracle" else "oceanbase.")
            + "v$sql_monitor_statname order by ID"
        )
    ...
    def otherstat_detail_explain_item(self, item, n, v):
        try:
            if 0 == item[n]:
                ...
                val = "%s.%06d" % (
                    time.strftime(
                        "%Y-%m-%d %H:%M:%S", time.localtime(item[v] / 1000000)
                    ),
        ...
        return (
            ""
            if item[n] == 0
            else self.STAT_NAME[item[n]]["name"] + "(" + val + ");<br/>"
        )
    ...
    def detail_otherstat_explain(self, item):
        otherstat = ""
        otherstat += self.otherstat_detail_explain_item(
            item, "OTHERSTAT_1_ID", "OTHERSTAT_1_VALUE"
        )
        otherstat += self.otherstat_detail_explain_item(
            item, "OTHERSTAT_2_ID", "OTHERSTAT_2_VALUE"
        )
        otherstat += self.otherstat_detail_explain_item(
            item, "OTHERSTAT_3_ID", "OTHERSTAT_3_VALUE"
        )
        otherstat += self.otherstat_detail_explain_item(
            item, "OTHERSTAT_4_ID", "OTHERSTAT_4_VALUE"
        )
        otherstat += self.otherstat_detail_explain_item(
            item, "OTHERSTAT_5_ID", "OTHERSTAT_5_VALUE"
        )
        otherstat += self.otherstat_detail_explain_item(
            item, "OTHERSTAT_6_ID", "OTHERSTAT_6_VALUE"
        )
        return otherstat

    def dfo_otherstat_explain(self, item):
        otherstat = ""
        otherstat += self.otherstat_agg_explain_item(
            item, "OTHERSTAT_1_ID", "SUM_STAT_1"
        )
        otherstat += self.otherstat_agg_explain_item(
            item, "OTHERSTAT_2_ID", "SUM_STAT_2"
        )
        otherstat += self.otherstat_agg_explain_item(
            item, "OTHERSTAT_3_ID", "SUM_STAT_3"
        )
        otherstat += self.otherstat_agg_explain_item(
            item, "OTHERSTAT_4_ID", "SUM_STAT_4"
        )
        otherstat += self.otherstat_agg_explain_item(
            item, "OTHERSTAT_5_ID", "SUM_STAT_5"
        )
        otherstat += self.otherstat_agg_explain_item(
            item, "OTHERSTAT_6_ID", "SUM_STAT_6"
        )
        return otherstat
    ...
    def report_detail_graph_data(self, ident, cursor, title=""):
        ...
        for item in cursor:
            ...
            data = (
                data
                + "{start:%f, end:%f, diff:%f, opid:%s, op:'%s',tid:'%s',rows:%d, tag:'op', depth:%d, rescan:%d, svr_ip:'%s', otherstat:'%s'},"
                % (
                    start,
                    end,
                    end - start,
    ...
    def report_detail_graph_data_obversion4(self, ident, cursor, title=""):
        ...
        for item in cursor:
            ...
            data = (
                data
                + "{cpu:%f, io:%f, start:%f, end:%f, diff:%f, opid:%s, op:'%s',tid:'%s',rows:%d, tag:'op', depth:%d, rescan:%d, svr_ip:'%s', otherstat:'%s'},"
                % (
                    item["MY_CPU_TIME"],
                    item["MY_IO_TIME"],
                    start,
    ...
    def report_dfo_agg_db_time_graph_data_obversion4(self, cursor, title=""):
        ...
        for item in cursor:
            ...
            data = (
                data
                + "{cpu:%f,io:%f,start:%f, end:%f, diff:%f, my_io_time:%f, my_cpu_time:%f, opid:%s, op:'%s', est_rows:0, rows:%d, tag:'db_time', tid: %d, depth:%d, otherstat:'%s'},"
                % (
                    item["MY_CPU_TIME"],
                    item["MY_IO_TIME"],
                    start,
        ...
        data = (
            data
            + "</script><p>%s</p><div class='bar' id='db_time_serial'></div>" % (title)
        )
        self.__report(data)
    ...
    def report_dfo_agg_graph_data(self, cursor, title=""):
        ...
        for item in cursor:
            start = (
                0
                if None == item["MIN_FIRST_CHANGE_TS"]
                else item["MIN_FIRST_CHANGE_TS"]
            )
            end = (
                0 if None == item["MAX_LAST_CHANGE_TS"] else item["MAX_LAST_CHANGE_TS"]
            )
            ...
            data = (
                data
                + "{start:%f, end:%f, diff:%f, opid:%s, op:'%s',tid:'%s',rows:%d,est_rows:%d, tag:'dfo', depth:%d, otherstat:'%s'},"
                % (
                    start,
                    end,
                    end - start,
        ...
        data = data + "{start:0}];"
        data = data + "</script><p>%s</p><div class='bar' id='agg_serial'></div>" % (
            title
        )
        self.__report(data)
    ...
    def report_dfo_agg_graph_data_obversion4(self, cursor, title=""):
        ...
        for item in cursor:
            ...
            data = (
                data
                + "{cpu:%f,io:%f,start:%f, end:%f, diff:%f, opid:%s, op:'%s',tid:'%s',rows:%d,est_rows:%d, tag:'dfo', depth:%d, otherstat:'%s', skewness:%.2f},"
                % (
                    item["MY_CPU_TIME"],
                    item["MY_IO_TIME"],
                    start,
    ...
    def report_dfo_sched_agg_graph_data(self, cursor, title=""):
        ...
        for item in cursor:
            start = (
                0
                if None == item["MIN_FIRST_REFRESH_TS"]
                else item["MIN_FIRST_REFRESH_TS"]
            )
            end = (
                0
                if None == item["MAX_LAST_REFRESH_TS"]
                else item["MAX_LAST_REFRESH_TS"]
            )
            rows = 0 if None == item["TOTAL_OUTPUT_ROWS"] else item["TOTAL_OUTPUT_ROWS"]
            est_rows = 0 if None == item["EST_ROWS"] else item["EST_ROWS"]
            otherstat = self.dfo_otherstat_explain(item)
            data = (
                data
                + "{start:%f, end:%f, diff:%f, opid:%s, op:'%s',tid:'%s',rows:%d,est_rows:%d, tag:'dfo', "
                "depth:%d, otherstat:'%s'},"
                % (
                    start,
                    end,
        ...
        data = (
            data
            + "</script><p>%s</p><div class='bar' id='agg_sched_serial'></div>"
            % (title)
        )
        self.__report(data)
    ...
    def report_dfo_sched_agg_graph_data_obversion4(self, cursor, title=""):
        ...
        for item in cursor:
            ...
            data = (
                data
                + "{cpu:%f,io:%f,start:%f, end:%f, diff:%f, opid:%s, op:'%s',tid:'%s',rows:%d,est_rows:%d, "
                "tag:'dfo', depth:%d, otherstat:'%s', skewness:%.2f},"
                % (
                    item["MY_CPU_TIME"],
                    item["MY_IO_TIME"],
                    start,
    ...
    def report_svr_agg_graph_data(self, ident, cursor, title=""):
        ...
        for item in cursor:
            ...
            data = (
                data
                + "{start:%f, end:%f, diff:%f, opid:%s, op:'%s',tid:'%s',svr:'%s',rows:%d, "
                "tag:'sqc', depth:%d},"
                % (
                    start,
                    end,
        ...
        data = data + "<p>%s</p><div class='bar' id='%s'></div>" % (title, ident)
        self.stdio.verbose(
            "report SQL_PLAN_MONITOR SQC operator priority start, DATA: %s", data
        )
        self.__report(data)
    ...
    def report_svr_agg_graph_data_obversion4(self, ident, cursor, title=""):
        ...
        for item in cursor:
            ...
            data = (
                data
                + "{cpu:%f,io:%f,start:%f, end:%f, diff:%f, opid:%s, op:'%s',tid:'%s',svr:'%s',rows:%d, "
                "tag:'sqc', depth:%d, skewness:%.2f},"
                % (
                    item["MY_CPU_TIME"],
                    item["MY_IO_TIME"],
                    start,
    ...
    def tenant_mode_detected(self):
        try:
            # Detect MySQL mode
            data = self.db_connector.execute_sql(
            ...
            if matched_version:
                ...
                self.sql_audit_name = (
                    "gv$ob_sql_audit" if major_version >= 4 else "gv$sql_audit"
                )
                self.plan_explain_name = (
                    "gv$ob_plan_cache_plan_explain"
                    if major_version >= 4
                    else "gv$plan_cache_plan_explain"
                )
                ...
                self.stdio.verbose(
                    f"Detected MySQL mode successful, Database version: {ob_version}"
                )
                return True
            else:
            ...
            # Detect Oracle mode
            try:
                data = self.sys_connector.execute_sql(
                    "select SUBSTR(BANNER, 11, 100) from V$VERSION;"
                )
                banner = data[0][0]
                ...
                if matched_version:
                    ...
                    self.sql_audit_name = (
                        f"gv$ob_sql_audit" if major_version >= 4 else "gv$sql_audit"
                    )
                    self.ob_major_version = major_version
                    ...
                    self.stdio.verbose(
                        f"Detected Oracle mode successful, Database version: {version}"
                    )
                    return True
                else:
    ...
    def init_resp(self):
        ...
        resp = {
            "skip": False,
            "error": False,
            "gather_pack_path": self.local_stored_path,
        }
        if resp["error"]:
            return resp
        return resp
    ...
    def sql_audit_by_trace_id_limit1_sql(self):
        if self.tenant_mode == "mysql":
            sql = (
                str(GlobalSqlMeta().get_value(key="sql_audit_by_trace_id_limit1_mysql"))
                .replace("##REPLACE_TRACE_ID##", self.trace_id)
                .replace("##REPLACE_SQL_AUDIT_TABLE_NAME##", self.sql_audit_name)
            )
        else:
            sql = (
                str(
                    GlobalSqlMeta().get_value(key="sql_audit_by_trace_id_limit1_oracle")
                )
                .replace("##REPLACE_TRACE_ID##", self.trace_id)
                .replace("##REPLACE_SQL_AUDIT_TABLE_NAME##", self.sql_audit_name)
            )
    ...
    def plan_explain_sql(self, tenant_id, plan_id, svr_ip, svr_port):
        if self.tenant_mode == "mysql":
            if self.ob_major_version >= 4:
                sql = (
                    "select * from oceanbase.gv$ob_plan_cache_plan_explain where tenant_id = %s and "
                    "plan_id = %s  and svr_ip = '%s' and svr_port = %s"
                    % (tenant_id, plan_id, svr_ip, svr_port)
                )
            else:
                sql = (
                    "select * from oceanbase.gv$plan_cache_plan_explain where tenant_id = %s and "
                    "plan_id = %s  and ip = '%s' and port = %s"
                    % (tenant_id, plan_id, svr_ip, svr_port)
                )
        else:
            if self.ob_major_version >= 4:
                sql = (
                    "select * from sys.gv$ob_plan_cache_plan_explain where tenant_id = %s and plan_id = "
                    "%s  and svr_ip = '%s' and svr_port = %s"
                    % (tenant_id, plan_id, svr_ip, svr_port)
                )
            else:
                sql = (
                    "select * from sys.gv$plan_cache_plan_explain where tenant_id = %s and plan_id = "
                    "%s  and svr_ip = '%s' and svr_port = %s"
                    % (tenant_id, plan_id, svr_ip, svr_port)
                )
        return sql
    ...
    def full_audit_sql_by_trace_id_sql(self, trace_id):
        if self.tenant_mode == "mysql":
            if self.ob_major_version >= 4:
                sql = (
                    "select /*+ sql_audit */ %s from oceanbase.%s where trace_id = '%s' "
                    "AND client_ip IS NOT NULL ORDER BY QUERY_SQL ASC, REQUEST_ID"
                    % (
                        GlobalSqlMeta().get_value(
                            key="sql_audit_item_mysql_obversion4"
                        ),
                        self.sql_audit_name,
                        trace_id,
                    )
                ...
                sql = (
                    ...
                    "AND client_ip IS NOT NULL ORDER BY QUERY_SQL ASC, REQUEST_ID"
                    % (
                        GlobalSqlMeta().get_value(key="sql_audit_item_mysql"),
                        self.sql_audit_name,
                        trace_id,
                    )
                )
            ...
            if self.ob_major_version >= 4:
                sql = (
                    "select /*+ sql_audit */ %s from sys.%s where trace_id = '%s' AND  "
                    "length(client_ip) > 4 ORDER BY  REQUEST_ID"
                    % (
                        GlobalSqlMeta().get_value(
                            key="sql_audit_item_oracle_obversion4"
                        ),
                        self.sql_audit_name,
                        trace_id,
                    )
                ...
                sql = (
                    ...
                    "length(client_ip) > 4 ORDER BY  REQUEST_ID"
                    % (
                        GlobalSqlMeta().get_value(key="sql_audit_item_oracle"),
                        self.sql_audit_name,
                        trace_id,
                    )
                )
        ...
        if self.tenant_mode == "mysql":
            if self.ob_major_version >= 4:
                sql = (
                    str(
                        GlobalSqlMeta().get_value(
                            key="sql_plan_monitor_dfo_op_mysql_obversion4"
                        )
                    )
                    .replace("##REPLACE_TRACE_ID##", trace_id)
                    ...
                    .replace(
                        "##REPLACE_PLAN_EXPLAIN_TABLE_NAME##", self.plan_explain_name
                    )
                )
            else:
                sql = (
    ...
    def sql_plan_monitor_svr_agg_template_sql(self):
        if self.tenant_mode == "mysql":
            if self.ob_major_version >= 4:
                sql = GlobalSqlMeta().get_value(
                    key="sql_plan_monitor_svr_agg_template_mysql_obversion4"
                )
            else:
                sql = GlobalSqlMeta().get_value(
                    key="sql_plan_monitor_svr_agg_template_mysql"
                )
        else:
            if self.ob_major_version >= 4:
                sql = GlobalSqlMeta().get_value(
                    key="sql_plan_monitor_svr_agg_template_oracle_obversion4"
                )
            else:
                sql = GlobalSqlMeta().get_value(
                    key="sql_plan_monitor_svr_agg_template_oracle"
                )
        return sql
    ...
    def sql_plan_monitor_detail_template_sql(self):
        if self.tenant_mode == "mysql":
            if self.ob_major_version >= 4:
                sql = GlobalSqlMeta().get_value(
                    key="sql_plan_monitor_detail_template_mysql_obversion4"
                )
            else:
                sql = GlobalSqlMeta().get_value(
                    key="sql_plan_monitor_detail_template_mysql"
                )
        else:
            if self.ob_major_version >= 4:
                sql = GlobalSqlMeta().get_value(
                    key="sql_plan_monitor_detail_template_oracle_obversion4"
                )
            else:
                sql = GlobalSqlMeta().get_value(
                    key="sql_plan_monitor_detail_template_oracle"
                )
        return sql
    ...
    def report_sql_audit_details(self, sql):
        if self.enable_dump_db:
            ...
            self.__report(
                "<div><h2 id='sql_audit_table_anchor'>SQL_AUDIT 信息</h2><div class='v' id='sql_audit_table' style='display: none'>"
                + full_audit_sql_result.get_html_string()
                + "</div></div>"
            )
    ...
    def report_plan_explain(self, db_name, raw_sql):
        ...
        try:
            sql_explain_cursor = self.db_connector.execute_sql_return_cursor(
                explain_sql
            )
    ...
    def report_sql_plan_monitor_dfo_op(self, sql):
        ...
        self.__report(
            "<div><h2 id='agg_table_anchor'>SQL_PLAN_MONITOR DFO 级调度时序汇总</h2><div class='v' id='agg_table' style='display: none'>"
            + data_sql_plan_monitor_dfo_op.get_html_string()
            + "</div></div>"
        )
        self.stdio.verbose("report SQL_PLAN_MONITOR DFO complete")
        cursor_sql_plan_monitor_dfo_op = (
            self.sys_connector.execute_sql_return_cursor_dictionary(sql)
        )
        if self.ob_major_version >= 4:
            self.report_dfo_sched_agg_graph_data_obversion4(
                cursor_sql_plan_monitor_dfo_op, "调度时序图"
            )
        else:
            self.report_dfo_sched_agg_graph_data(
                cursor_sql_plan_monitor_dfo_op, "调度时序图"
            )
        ...
        if self.ob_major_version >= 4:
            self.report_dfo_agg_graph_data_obversion4(
                cursor_sql_plan_monitor_dfo_op, "数据时序图"
            )
        else:
    ...
    def report_sql_plan_monitor_svr_agg(
        self, sql_plan_monitor_svr_agg_v1, sql_plan_monitor_svr_agg_v2
    ):
        cursor_sql_plan_monitor_svr_agg = self.sys_connector.execute_sql_return_cursor(
            sql_plan_monitor_svr_agg_v1
        )
        self.__report(
        ...
        cursor_sql_plan_monitor_svr_agg_v1 = (
            self.sys_connector.execute_sql_return_cursor_dictionary(
                sql_plan_monitor_svr_agg_v2
            )
        )
        if self.ob_major_version >= 4:
            self.report_svr_agg_graph_data_obversion4(
                "svr_agg_serial_v1", cursor_sql_plan_monitor_svr_agg_v1, "算子优先视图"
            )
        else:
            self.report_svr_agg_graph_data(
                "svr_agg_serial_v1", cursor_sql_plan_monitor_svr_agg_v1, "算子优先视图"
            )
        self.stdio.verbose("report SQL_PLAN_MONITOR SQC operator priority complete")
        cursor_data_sql_plan_monitor_svr_agg_v2 = (
            self.sys_connector.execute_sql_return_cursor_dictionary(
                sql_plan_monitor_svr_agg_v2
            )
        )
    ...
    def report_sql_plan_monitor_detail_operator_priority(self, sql):
        cursor_sql_plan_monitor_detail = self.sys_connector.execute_sql_return_cursor(
            sql
        )
        self.__report(
            "<div><h2 id='detail_table_anchor'>SQL_PLAN_MONITOR 详情</h2><div class='v' id='detail_table' style='display: none'>"
            + (
                "no result in --fast mode"
                if self.enable_fast_dump
                else from_db_cursor(cursor_sql_plan_monitor_detail).get_html_string()
            )
        ...
        if self.ob_major_version >= 4:
            self.report_detail_graph_data_obversion4(
                "detail_serial_v1", cursor_sql_plan_monitor_detail_v1, "算子优先视图"
            )
        else:
            self.report_detail_graph_data(
                "detail_serial_v1", cursor_sql_plan_monitor_detail_v1, "算子优先视图"
            )
    ...
    def reportsql_plan_monitor_detail_svr_priority(self, sql):
        ...
        if self.ob_major_version >= 4:
            self.report_detail_graph_data_obversion4(
                "detail_serial_v2", cursor_sql_plan_monitor_detail_v2, "线程优先视图"
            )
        else:
            self.report_detail_graph_data(
                "detail_serial_v2", cursor_sql_plan_monitor_detail_v2, "线程优先视图"
