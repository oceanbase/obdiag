...
class GatherObProxyLogHandler(BaseShellHandler):
    ...
    def __get_log_name(self, ssh_helper, node):
        ...
        if self.scope == "obproxy" or self.scope == "obproxy_stat" or self.scope == "obproxy_digest" or \
            ...
            get_obproxy_log = "ls -1 -F %s/obproxy.*log* %s/obproxy_error.*log* %s/obproxy_stat.*log* %s/obproxy_digest.*log* %s/obproxy_limit.*log* %s/obproxy_slow.*log* | awk -F '/' '{print $NF}'" % (log_path, log_path, log_path, log_path, log_path, log_path)
        if self.is_ssh:
