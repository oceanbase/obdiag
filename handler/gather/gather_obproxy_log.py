...
class GatherObProxyLogHandler(BaseShellHandler):
    ...
    def __handle_from_node(self, node, pack_dir_this_command):
        ...
        if not ssh_failed:
            ...
            gather_dir_name = "obproxy_log_{0}_{1}_{2}".format(
            ...
            if is_empty:
                ...
                self.__handle_zip_file(
                    remote_ip, ssh, resp, gather_dir_name, pack_dir_this_command
                )
            ssh.ssh_close()
        return resp
    ...
    def __get_log_name(self, ssh_helper, node):
        ...
        if (
            self.scope == "obproxy"
            or self.scope == "obproxy_stat"
            or self.scope == "obproxy_digest"
            or self.scope == "obproxy_limit"
            or self.scope == "obproxy_slow"
            or self.scope == "obproxy_diagnosis"
            or self.scope == "obproxy_error"
        ):
            get_obproxy_log = "ls -1 -F %s/*%s.*log* | awk -F '/' '{print $NF}'" % (
                log_path,
                self.scope,
            )
        else:
            get_obproxy_log = (
                "ls -1 -F %s/obproxy.*log* %s/obproxy_error.*log* %s/obproxy_stat.*log* %s/obproxy_digest.*log* %s/obproxy_limit.*log* %s/obproxy_slow.*log* %s/obproxy_diagnosis.*log*| awk -F '/' '{print $NF}'"
        ...
        if log_files:
            ...
            self.stdio.error(
                "Unable to find the log file. Please provide the correct home_path config and check obproxy {0} log exist".format(
                    log_path
                )
            )
    ...
    def __handle_zip_file(self, ip, ssh, resp, gather_dir_name, pack_dir_this_command):
        ...
        gather_dir_full_path = "{0}/{1}".format(
            self.gather_log_temporary_dir, gather_dir_name
        )
        if self.zip_encrypt:
