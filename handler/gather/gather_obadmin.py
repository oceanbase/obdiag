...
class GatherObAdminHandler(BaseShellHandler):
    ...
    def init_option(self):
        ...
        if from_option is not None and to_option is not None:
            ...
            self.from_time_str = (
                now_time
                - datetime.timedelta(
                    seconds=TimeUtils.parse_time_length_to_sec(since_option, self.stdio)
                )
            ...
            if since_option is not None:
                self.from_time_str = (
                    now_time
                    - datetime.timedelta(
                        seconds=TimeUtils.parse_time_length_to_sec(
                            since_option, self.stdio
                        )
                    )
                ).strftime("%Y-%m-%d %H:%M:%S")
            else:
    ...
    def handle(self):
        ...
        if self.ob_admin_mode == "slog":
            ...
            mode = "clog"
        summary_tuples = self.__get_overall_summary(
            gather_tuples, mode, self.zip_encrypt
        )
    ...
    def __handle_from_node(self, local_stored_path, node):
        ...
        if not ssh_failed:
            ...
            (
                SshClient(self.stdio).run(ssh_helper, mkdir_cmd)
                if self.is_ssh
                else LocalClient(self.stdio).run(mkdir_cmd)
            )
            ...
            if (
                ob_version != ""
                and not StringUtils.compare_versions_lower(
                    ob_version, const.MAX_OB_VERSION_SUPPORT_GATHER_OBADMIN, self.stdio
                )
            ) or ob_version == "":
                self.stdio.verbose(
                    "This version {0} does not support gather clog/slog . The max supported version less than {1}".format(
                        ob_version, const.MAX_OB_VERSION_SUPPORT_GATHER_OBADMIN
                    )
                )
            ...
            if is_empty_dir(
                ...
                resp = self.__handle_zip_file(
                    remote_ip, ssh_helper, resp, remote_dir_name, local_stored_path
                )
    ...
    def __handle_zip_file(self, ip, ssh, resp, gather_dir_name, pack_dir_this_command):
        ...
        self.stdio.print(
            FileUtil.show_file_size_tabulate(ip, gather_log_file_size, self.stdio)
        )
        local_path = ""
        if int(gather_log_file_size) < self.file_size_limit:
            local_path = download_file(
                self.is_ssh, ssh, gather_package_dir, pack_dir_this_command, self.stdio
            )
            resp["error"] = ""
    ...
    def __get_log_name(self, ssh_helper, node):
        ...
        if self.ob_admin_mode == "slog":
            get_log = "ls -l SLOG_DIR --time-style '+.%Y%m%d%H%M%S' | awk '{print $7,$6}'".replace(
                "SLOG_DIR", slog_dir
            )
        else:
            get_log = "ls -l CLOG_DIR --time-style '+.%Y%m%d%H%M%S' | awk '{print $7,$6}'".replace(
                "CLOG_DIR", clog_dir
            )
        log_files = (
            SshClient(self.stdio).run(ssh_helper, get_log)
            if self.is_ssh
            else LocalClient(self.stdio).run(get_log)
        )
        log_name_list = []
        for file_name in log_files.split("\n"):
            ...
            if (
                ...
                log_time = datetime.datetime.strptime(
                    log_name_fields[-1], "%Y%m%d%H%M%S"
                )
                from_time = datetime.datetime.strptime(
                    self.from_time_str, "%Y-%m-%d %H:%M:%S"
                )
                to_time = datetime.datetime.strptime(
                    self.to_time_str, "%Y-%m-%d %H:%M:%S"
                )
    ...
    def __mv_log(self, ssh_helper, remote_dir):
        if self.ob_admin_mode == "slog":
            cmd = "cd {remote_dir} && mv ob_admin.log ob_admin_slog.log".format(
                remote_dir=remote_dir
            )
        else:
            cmd = "cd {remote_dir} && mv ob_admin.log ob_admin_clog.log".format(
                remote_dir=remote_dir
            )
