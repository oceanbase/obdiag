...
class GatherAshReportHandler(SafeStdio):
    ...
    def version_check(self):
        ...
        try:
            ...
            if len(self.observer_nodes) > 0:
                ssher = SshHelper(
                    self.observer_nodes[0]["ip"],
                    self.observer_nodes[0]["ssh_port"],
                    self.observer_nodes[0]["ssh_username"],
                    self.observer_nodes[0]["ssh_password"],
                )
                observer_version = get_observer_version(
                    True, ssher, self.observer_nodes[0]["home_path"], self.stdio
                )
            else:
                self.stdio.warn(
                    "RCAHandler Failed to get observer version:{0}".format(e)
                )
        self.stdio.verbose(
            "RCAHandler.init get observer version: {0}".format(observer_version)
        )
        ...
        if not (
            ...
            self.stdio.error(
                "observer version: {0}, must greater than 4.0.0.0".format(
                    observer_version
                )
            )
            return False
    ...
    def execute(self):
        try:
            ...
            ash_report_data = self.obconn.callproc(
                "DBMS_WORKLOAD_REPOSITORY.ASH_REPORT", args=ash_report_arg
            )
            ...
            self.ash_report_file_name = os.path.join(
                self.report_path, self.ash_report_file_name
            )

            with open(self.ash_report_file_name, "w+") as f:
                f.write(ash_report)
            self.stdio.print(
                "save ash report file name: "
                + Fore.YELLOW
                + "{0}".format(self.ash_report_file_name)
                + Style.RESET_ALL
            )
            self.result_summary_file_name = os.path.join(
                self.report_path, "result_summary.txt"
            )
    ...
    def init_option(self):
        ...
        if from_option is not None and to_option is not None:
            ...
            self.to_time_str = (now_time + datetime.timedelta(minutes=0)).strftime(
                "%Y-%m-%d %H:%M:%S"
            )
            self.from_time_str = (
                now_time
            ...
            self.from_time_str = (now_time - datetime.timedelta(minutes=30)).strftime(
                "%Y-%m-%d %H:%M:%S"
            )
            self.stdio.print(
                "gather from_time: {0}, to_time: {1}".format(
        ...
        if report_type_option:
            ...
            if report_type_option.upper() != "TEXT":
                self.stdio.error(
                    "Invalid argument for report type, Now just support TEXT"
                )
                return False
        else:
            self.report_type = None
        ...
        if store_dir_option:
            ...
            self.gather_pack_dir = "./"
        self.stdio.print(
            "from_time: {0}, to_time: {1}, sql_id: {2}, trace_id: {3}, report_type: {4}, wait_class: {5}, store_dir: {6}".format(
                self.from_time_str,
                self.to_time_str,
                self.sql_id,
                self.trace_id,
