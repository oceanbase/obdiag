...
class GatherOsInfoHandler(BaseShellHandler):
    ...
    def __gather_dmesg_current_info(self, ssh_helper, gather_path):
        try:
            ...
            self.stdio.verbose(
                "gather dmesg current info, run cmd = [{0}]".format(dmesg_cmd)
            )
            (
                SshClient(self.stdio).run(ssh_helper, dmesg_cmd)
                if self.is_ssh
                else LocalClient(self.stdio).run(dmesg_cmd)
            )
        except:
            self.stdio.error(
                "Failed to gather dmesg current info on server {0}".format(
                    ssh_helper.get_name()
                )
            )
    ...
    def __gather_dmesg_boot_info(self, ssh_helper, dir_path):
        try:
            file_exit_cmd = "ls -l {file_path} 2>/dev/null".format(
                file_path="/var/log/dmesg"
            )
            file_exit = (
                SshClient(self.stdio).run(ssh_helper, file_exit_cmd)
                if self.is_ssh
                else LocalClient(self.stdio).run(file_exit_cmd)
            )
            if file_exit:
                dmesg_cmd = "cp --force /var/log/dmesg {dir_path}/dmesg.boot".format(
                    dir_path=dir_path
                )
                self.stdio.verbose(
                    "gather dmesg boot info on server {0}, run cmd = [{1}]".format(
                        ssh_helper.get_name(), dmesg_cmd
                    )
                )
                (
                    SshClient(self.stdio).run(ssh_helper, dmesg_cmd)
                ...
                self.stdio.warn(
                    "the file /var/log/dmesg on server {0} not found ".format(
                        ssh_helper.get_name()
                    )
                )
        except:
            self.stdio.error(
                "Failed to gather the /var/log/dmesg on server {0}".format(
                    ssh_helper.get_name()
                )
            )
    ...
    def __gather_cpu_info(self, ssh_helper, gather_path):
        try:
            ...
            self.stdio.verbose(
                "gather cpu info on server {0}, run cmd = [{1}]".format(
                    ssh_helper.get_name(), tsar_cmd
                )
            )
            (
                SshClient(self.stdio).run(ssh_helper, tsar_cmd)
                if self.is_ssh
                else LocalClient(self.stdio).run(tsar_cmd)
            )
        except:
            self.stdio.error(
                "Failed to gather cpu info use tsar on server {0}".format(
                    ssh_helper.get_name()
                )
            )
    ...
    def __gather_mem_info(self, ssh_helper, gather_path):
        try:
            ...
            self.stdio.verbose(
                "gather memory info on server {0}, run cmd = [{1}]".format(
                    ssh_helper.get_name(), tsar_cmd
                )
            )
            (
            ...
            self.stdio.error(
                "Failed to gather memory info use tsar on server {0}".format(
                    ssh_helper.get_name()
                )
            )
    ...
    def __gather_swap_info(self, ssh_helper, gather_path):
        try:
            ...
            self.stdio.verbose(
                "gather swap info on server {0}, run cmd = [{1}]".format(
                    ssh_helper.get_name(), tsar_cmd
                )
            )
            (
            ...
            self.stdio.error(
                "Failed to gather swap info use tsar on server {0}".format(
                    ssh_helper.get_name()
                )
            )
    ...
    def __gather_io_info(self, ssh_helper, gather_path):
        try:
            ...
            self.stdio.verbose(
                "gather io info on server {0}, run cmd = [{1}]".format(
                    ssh_helper.get_name(), tsar_cmd
                )
            )
            (
            ...
            self.stdio.error(
                "Failed to gather io info use tsar on server {0}".format(
                    ssh_helper.get_name()
                )
            )
    ...
    def __gather_traffic_info(self, ssh_helper, gather_path):
        try:
            ...
            self.stdio.verbose(
                "gather traffic info on server {0}, run cmd = [{1}]".format(
                    ssh_helper.get_name(), tsar_cmd
                )
            )
            (
            ...
            self.stdio.error(
                "Failed to gather traffic info use tsar on server {0}".format(
                    ssh_helper.get_name()
                )
            )
    ...
    def __gather_tcp_udp_info(self, ssh_helper, gather_path):
        try:
            tsar_cmd = (
                "tsar  --tcp --udp -d 1 > {gather_path}/tsar_tcp_udp_data.txt".format(
                    gather_path=gather_path
                )
            )
            self.stdio.verbose(
                "gather tcp and udp info on server {0}, run cmd = [{1}]".format(
                    ssh_helper.get_name(), tsar_cmd
                )
            )
            (
            ...
            self.stdio.error(
                "Failed to gather tcp and udp info use tsar on server {0}".format(
                    ssh_helper.get_name()
                )
            )
