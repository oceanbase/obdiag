...
class Base(SafeStdio):
    ...
    def execute(self):
        ...
        try:
            ...
            if (
                (self.node_number > 1)
                and self.step.get("global")
                and (self.step.get("global") is True)
            ):
                self.stdio.verbose(
                    "step sets the value of the global is true and it is processing the {0} node, skipping gather".format(
                        self.node_number
                    )
                )
            else:
                if self.step["type"] == "ssh":
                    ...
                    if (
                        self.node.get("host_type")
                        and self.node.get("host_type") == "OBSERVER"
                    ):
                        handler = GatherLogHandler(
                            self.context,
                        ...
                        self.stdio.verbose(
                            "node host_type is {0} not OBSERVER, skipping gather log".format(
                                self.node.get("host_type")
                            )
                        )
                    ...
                    if (
                        self.node.get("host_type")
                        and self.node.get("host_type") == "OBPROXY"
                    ):
                        ...
                        self.stdio.verbose(
                            "node host_type is {0} not OBPROXY, skipping gather log".format(
                                self.node.get("host_type")
                            )
                        )
