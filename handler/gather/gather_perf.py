...
class GatherPerfHandler(BaseShellHandler):
    ...
    def __handle_from_node(self, node, local_stored_path):
        ...
        if not ssh_failed:
            ...
            pid_observer_list = get_observer_pid(
            ...
            for pid_observer in pid_observer_list:
                if self.scope == "sample":
                    self.__gather_perf_sample(
                        ssh_helper, remote_dir_full_path, pid_observer
                    )
                elif self.scope == "flame":
                    self.__gather_perf_flame(
                        ssh_helper, remote_dir_full_path, pid_observer
                    )
                else:
                    self.__gather_perf_sample(
    ...
    def __gather_perf_sample(self, ssh_helper, gather_path, pid_observer):
        try:
            ...
            self.stdio.verbose(
                "generate perf sample data, run cmd = [{0}]".format(generate_data)
            )
            (
                SshClient(self.stdio).run_ignore_err(ssh_helper, generate_data)
                if self.is_ssh
                else LocalClient(self.stdio).run(generate_data)
            )
        except:
            self.stdio.error(
                "generate perf sample data on server [{0}] failed".format(
                    ssh_helper.get_name()
                )
            )
    ...
    def __gather_perf_flame(self, ssh_helper, gather_path, pid_observer):
        try:
            ...
            (
                SshClient(self.stdio).run_ignore_err(ssh_helper, perf_cmd)
                if self.is_ssh
                else LocalClient(self.stdio).run(perf_cmd)
            )

            generate_data = (
                "cd {gather_path} && perf script -i flame.data > flame.viz".format(
                    gather_path=gather_path
                )
            )
            self.stdio.verbose(
                "generate perf data, run cmd = [{0}]".format(generate_data)
            )
            (
                SshClient(self.stdio).run_ignore_err(ssh_helper, generate_data)
                if self.is_ssh
            ...
            self.stdio.error(
                "generate perf data on server [{0}] failed".format(
                    ssh_helper.get_name()
                )
            )
    ...
    def __gather_top(self, ssh_helper, gather_path, pid_observer):
        try:
            ...
            self.stdio.error(
                "gather top on server failed [{0}]".format(ssh_helper.get_name())
            )
