...
class GatherScenesListHandler:
    def __init__(self, yaml_tasks_base_path="./handler/gather/tasks/"):
        self.observer_tasks = {}
        self.obproxy_tasks = {}
        self.other_tasks = {}
        ...
        if os.path.exists(base_path):
            ...
            logger.error("Failed to find yaml task path: {0}".format(base_path))

    def handle(self, args):
        logger.debug("list gather scene")
        self.get_all_yaml_tasks()
        self.get_all_code_tasks()
        logger.debug("len of observer_tasks: {0}; len of observer_tasks: {1}; len of observer_tasks: {2};".format(len(self.observer_tasks), len(self.obproxy_tasks), len(self.other_tasks)))
        if (len(self.observer_tasks) + len(self.obproxy_tasks) + len(self.other_tasks)) == 0:
            logger.error("Failed to find any tasks")
        else:
    ...
    def get_all_yaml_tasks(self):
        try:
            ...
            logger.error("get all yaml task failed, error: ", e)
    ...
    def get_all_code_tasks(self):
        try:
            ...
            logger.error("get all hard code task failed, error: ", e)
    ...
    def get_one_yaml_task(self, name):
        try:
            ...
            logger.error("get one yaml task failed, error: ", e)
    ...
    def is_code_task(self, name):
        try:
            ...
            logger.error("get one code task failed, error: ", e)
            return False
    ...
    def print_scene_data(self):
        ...
        if self.other_tasks:
            ...
            print_title("Other Problem Gather Scenes")
            print_scene(sorted_other_tasks_dict)
        if self.obproxy_tasks:
            ...
            print_title("Obproxy Problem Gather Scenes")
            print_scene(sorted_obproxy_tasks_dict)
        if self.observer_tasks:
            ...
            print_title("Observer Problem Gather Scenes")
            print_scene(sorted_observer_tasks_dict)
