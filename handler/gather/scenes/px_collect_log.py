...
from common.command import (
    uzip_dir_local,
    analyze_log_get_sqc_addr,
    delete_file_in_folder,
    find_home_path_by_port,
)
...
class SQLPXCollectLogScene(object):
    ...
    def __gather_log(self):
        try:
            self.stdio.verbose(
                "gather observer log start, trace id: {0}".format(self.trace_id)
            )
            ...
            from_time_str = (self.search_time - datetime.timedelta(days=3)).strftime(
                "%Y-%m-%d %H:%M:%S"
            )
            to_time_str = (self.search_time + datetime.timedelta(minutes=1)).strftime(
                "%Y-%m-%d %H:%M:%S"
            )
    ...
    def __analyze_log(self):
        try:
            ...
            if ip_port_str is None or len(ip_port_str) == 0:
                self.stdio.warn(
                    "No logs were found indicating that the SQC interrupted the QC; the error occurred locally in the QC."
                )
                self.sql_task_node = []
                return
            ...
            if self.sql_task_node == sqc_sql_task_node:
                self.stdio.verbose(
                    "The SQC with an error occurred on the same observer as the QC."
                )
                self.sql_task_node = []
            else:
                self.stdio.verbose(
                    "The SQC with an error info: {0}".format(sqc_sql_task_node)
                )
        ...
        for node in self.ob_nodes:
            if node["ip"] == ip_str:
                ...
                try:
                    ...
                    self.stdio.error(
                        "ssh {0}@{1}: failed, Please check the config".format(
    ...
    def __parse_env(self):
        try:
            ...
            if search_time is None or len(search_time) == 0:
                ...
                try:
                    search_time = datetime.datetime.strptime(
                        search_time, "%Y-%m-%d %H:%M:%S"
                    )
                except Exception as e:
                    self.stdio.error(
                        "option env [--estimated_time] format error, please use '%Y-%m-%d %H:%M:%S', run 'obdiag gather scene list' to check usage"
                    )
                    return False
