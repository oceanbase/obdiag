...
hardcode_scene_list = [
    RegisteredHardCodeScene(
        'observer.perf_sql',
        f'''obdiag gather scene run --scene=observer.perf_sql --env "{{db_connect='{db_connect}', trace_id='{trace_id}'}}"''',
        '[SQL performance problem]',
        '[SQL性能问题]'
    ),
    RegisteredHardCodeScene(
        'observer.sql_err',
        f'''obdiag gather scene run --scene=observer.sql_err --env "{{db_connect='{db_connect}', trace_id='{trace_id}'}}"''',
        '[SQL execution error]',
        '[SQL 执行出错]'
    ),
