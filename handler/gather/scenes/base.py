...
class SceneBase(SafeStdio):
    ...
    def __execute_yaml_mode(self, nodes):
        ...
        for node in nodes:
            self.stdio.print(
                "run scene excute yaml mode in node: {0} start".format(
                    StringUtils.node_cut_passwd_for_log(node["ip"], self.stdio)
                )
            )
            ...
            for step in steps["steps"]:
                ...
                nu = nu + 1
            self.stdio.print(
                "run scene excute yaml mode in node: {0} end".format(
                    StringUtils.node_cut_passwd_for_log(node["ip"], self.stdio)
                )
            )
    ...
    def __execute_code_mode(self):
        if (
            self.scene["name"] == "observer.perf_sql"
            or self.scene["name"] == "observer.sql_err"
        ):
            scene = SQLProblemScene(
                self.context,
                self.scene["name"],
                self.report_dir,
                self.scene_variable_dict,
            ...
            scene = CPUHighScene(
                self.context, self.report_dir, self.scene_variable_dict, self.env
            )
        elif self.scene["name"] == "observer.px_collect_log":
            scene = SQLPXCollectLogScene(
                self.context,
                self.scene["name"],
                self.report_dir,
                self.scene_variable_dict,
            ...
            self.stdio.error(
                "unsupported hard code scene {0}".format(self.scene["name"])
            )
            return
        try:
            self.stdio.verbose(
                "hard code scene {0} execute start".format(self.scene["name"])
            )
            scene.execute()
            self.stdio.verbose(
                "hard code scene {0} execute end".format(self.scene["name"])
            )
        except Exception as e:
