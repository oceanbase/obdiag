...
class GatherObstack2Handler(BaseShellHandler):
    ...
    def __handle_from_node(self, local_stored_path, node):
        ...
        if not ssh_failed:
            if not is_support_arch(self.is_ssh, ssh_helper, self.stdio):
                resp["error"] = (
                    "remote server {0} arch not support gather obstack".format(
                        ssh_helper.get_name()
                    )
                )
                return resp
            ...
            ob_version = get_observer_version(
                self.is_ssh, ssh_helper, node.get("home_path"), self.stdio
            )
            if not StringUtils.compare_versions_greater(
                ob_version, const.MIN_OB_VERSION_SUPPORT_GATHER_OBSTACK
            ):
                self.stdio.verbose(
                    "This version {0} does not support gather obstack . The minimum supported version is {1}".format(
                        ob_version, const.MIN_OB_VERSION_SUPPORT_GATHER_OBSTACK
                    )
                )
            ...
            if is_need_install_obstack:
                self.stdio.verbose(
                    "There is no obstack2 on the host {0}. It needs to be installed. "
                ...
                obstack2_local_stored_full_path = os.path.join(
                    absPath, const.OBSTACK2_LOCAL_STORED_PATH
                )
                upload_file(
                    self.is_ssh,
                    ssh_helper,
                    obstack2_local_stored_full_path,
                    const.OBSTACK2_DEFAULT_INSTALL_PATH,
                    self.stdio,
                )
                self.stdio.verbose(
                    "Installation of obstack2 is completed and gather begins ..."
                )

            self.__chmod_obstack2(self.is_ssh, ssh_helper)
            # get observer_pid
            observer_pid_list = get_observer_pid(
                self.is_ssh, ssh_helper, node.get("home_path"), self.stdio
            )
            # gather obstack2 info
            for observer_pid in observer_pid_list:
                user = self.__get_observer_execute_user(ssh_helper, observer_pid)
                self.__gather_obstack2_info(
                    self.is_ssh, ssh_helper, user, observer_pid, remote_dir_name, node
                )
                try:
                    ...
                    self.stdio.error(
                        "Gather obstack info on the host {0} observer pid {1}".format(
                            remote_ip, observer_pid
                        )
                    )
                    delete_file_force(
                        self.is_ssh,
                        ssh_helper,
                        "/tmp/{dir_name}/observer_{pid}_obstack.txt".format(
                            dir_name=remote_dir_name, pid=observer_pid
                        ),
                        self.stdio,
                    )
                    pass
            if is_empty_dir(
                self.is_ssh, ssh_helper, "/tmp/{0}".format(remote_dir_name), self.stdio
            ):
            ...
            file_size = get_file_size(
                self.is_ssh, ssh_helper, remote_zip_file_path, self.stdio
            )
            remote_file_full_path = "{0}.zip".format(remote_dir_full_path)
            if int(file_size) < self.file_size_limit:
                local_file_path = "{0}/{1}.zip".format(
                    local_stored_path, remote_dir_name
                )
                download_file(
                    self.is_ssh,
                    ssh_helper,
            ...
            delete_file_force(
                self.is_ssh, ssh_helper, remote_file_full_path, self.stdio
            )
            ssh_helper.ssh_close()
            resp["gather_pack_path"] = "{0}/{1}.zip".format(
                local_stored_path, remote_dir_name
            )
        return resp
    ...
    def is_ready(self, ssh_helper, pid, remote_dir_name):
        try:
            self.stdio.verbose(
                "Check whether the directory /tmp/{dir_name} or "
                "file /tmp/{dir_name}/observer_{pid}_obstack.txt is empty".format(
                    dir_name=remote_dir_name, pid=pid
                )
            )
            is_empty_dir_res = is_empty_dir(
                self.is_ssh, ssh_helper, "/tmp/{0}".format(remote_dir_name), self.stdio
            )
            is_empty_file_res = is_empty_file(
                self.is_ssh,
                ssh_helper,
                "/tmp/{dir_name}/observer_{pid}_obstack.txt".format(
                    dir_name=remote_dir_name, pid=pid
            ...
            if is_empty_dir_res or is_empty_file_res:
                self.stdio.verbose(
                    ...
                    " is empty, waiting for the collection to complete".format(
                        host_ip=(
                            ssh_helper.get_name()
                            if self.is_ssh
                            else NetUtils.get_inner_ip(self.stdio)
                        ),
    ...
    def __chmod_obstack2(self, is_ssh, ssh_helper):
        ...
        (
            SshClient(self.stdio).run(ssh_helper, cmd)
            if is_ssh
            else LocalClient(self.stdio).run(cmd)
        )

    def __is_obstack_exists(self, is_ssh, ssh_helper):
        cmd = "test -e {file} && echo exists".format(
            file=const.OBSTACK2_DEFAULT_INSTALL_PATH
        )
        stdout = (
            SshClient(self.stdio).run(ssh_helper, cmd)
            if is_ssh
    ...
    def __get_observer_execute_user(self, ssh_helper, pid):
        cmd = "ps -o ruser=userForLongName -e -o pid,ppid,c,stime,tty,time,cmd | grep observer | grep {0} | awk {1}".format(
            pid, "'{print $1}'"
        )
        stdout = (
            SshClient(self.stdio).run(ssh_helper, cmd)
        ...
        self.stdio.verbose(
            "get observer execute user, run cmd = [{0}], result:{1} ".format(cmd, user)
        )
        return user

    def __gather_obstack2_info(
        self, is_ssh, ssh_helper, user, observer_pid, remote_gather_dir, node
    ):
        ...
        if is_ssh:
            if user == ssh_helper.username:
                ...
                chown_cmd = "chown {user} /tmp/{gather_dir}/".format(
                    user=user, gather_dir=remote_gather_dir
                )
                SshClient(self.stdio).run(ssh_helper_new, chown_cmd)
                self.stdio.verbose(
                    "gather obstack info on server {0}, run cmd = [su {1}, {2}]".format(
                        ssh_helper.get_name(), user, cmd
                    )
                )
