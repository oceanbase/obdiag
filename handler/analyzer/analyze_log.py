...
class AnalyzeLogHandler(BaseShellHandler):
    ...
    def init_option(self):
        ...
        if from_option is not None and to_option is not None:
            ...
            self.stdio.print(
                "analyze log from_time: {0}, to_time: {1}".format(
        ...
        if store_dir_option is not None:
            if not os.path.exists(os.path.abspath(store_dir_option)):
                self.stdio.warn(
                    "Error: args --store_dir [{0}] incorrect: No such directory, Now create it".format(
    ...
    def handle(self):
        ...
        if not self.init_config():
            ...
            return False
        local_store_parent_dir = os.path.join(
            self.gather_pack_dir,
            "analyze_pack_{0}".format(
        ...
        title, field_names, summary_list, summary_details_list = (
            self.__get_overall_summary(analyze_tuples, self.directly_analyze_files)
        )
        table = tabulate.tabulate(
            summary_list, headers=field_names, tablefmt="grid", showindex=False
        )
        self.stdio.stop_loading("analyze result sucess")
        self.stdio.print(title)
        self.stdio.print(table)
        FileUtil.write_append(
            os.path.join(local_store_parent_dir, "result_details.txt"),
            title + str(table) + "\n\nDetails:\n\n",
        )
        ...
        for m in range(len(summary_details_list)):
            for n in range(len(field_names)):
                ...
                FileUtil.write_append(
                    os.path.join(local_store_parent_dir, "result_details.txt"),
                    field_names[n] + ": " + str(summary_details_list[m][n]) + extend,
                )
        last_info = (
    ...
    def __handle_from_node(self, node, local_store_parent_dir):
        ...
        if "ssh_type" in node and node["ssh_type"] == "docker":
            local_store_dir = "{0}/docker_{1}".format(
                local_store_parent_dir, node["container_name"]
            )
        else:
            local_store_dir = "{0}/{1}".format(
                local_store_parent_dir, remote_ip.replace(".", "_")
            )
        ...
        try:
            ...
            self.stdio.error(
                "ssh {0}@{1}: failed, Please check the {2}".format(
                    remote_user, remote_ip, self.config_path
                )
            )
            ssh_failed = True
            resp["skip"] = True
        ...
        if not ssh_failed:
            from_datetime_timestamp = TimeUtils.timestamp_to_filename_time(
                TimeUtils.datetime_to_timestamp(self.from_time_str)
            )
            to_datetime_timestamp = TimeUtils.timestamp_to_filename_time(
                TimeUtils.datetime_to_timestamp(self.to_time_str)
            )
            gather_dir_name = "ob_log_{0}_{1}_{2}".format(
                ssh.host_ip, from_datetime_timestamp, to_datetime_timestamp
            )
            ...
            self.stdio.print(
                FileUtil.show_file_list_tabulate(remote_ip, log_list, self.stdio)
            )
            for log_name in log_list:
                if self.directly_analyze_files:
                    self.__pharse_offline_log_file(
                        ssh_helper=ssh,
                        log_name=log_name,
                        local_store_dir=local_store_dir,
                    )
                    analyze_log_full_path = "{0}/{1}".format(
                        local_store_dir, str(log_name).strip(".").replace("/", "_")
                    )
                else:
                    self.__pharse_log_file(
                        ssh_helper=ssh,
                        node=node,
                        log_name=log_name,
    ...
    def __handle_log_list(self, ssh, node, resp):
        ...
        if len(log_list) > self.file_number_limit:
            self.stdio.warn(
                "{0} The number of log files is {1}, out of range (0,{2}]".format(
                    node.get("ip"), len(log_list), self.file_number_limit
                )
            )
            resp["skip"] = (True,)
            resp["error"] = (
                "Too many files {0} > {1}, Please adjust the analyze time range".format(
                    len(log_list), self.file_number_limit
                )
            )
            if self.directly_analyze_files:
                resp["error"] = (
                    "Too many files {0} > {1}, "
                    "Please adjust the number of incoming files".format(
                        len(log_list), self.file_number_limit
                    )
                )
    ...
    def __get_log_name_list(self, ssh_helper, node):
        ...
        if (
            self.scope == "observer"
            or self.scope == "rootservice"
            or self.scope == "election"
        ):
            get_oblog = "ls -1 -F %s/*%s.log* | awk -F '/' '{print $NF}'" % (
                log_path,
                self.scope,
            )
        else:
            get_oblog = (
                "ls -1 -F %s/observer.log* %s/rootservice.log* %s/election.log* | awk -F '/' '{print $NF}'"
                % (log_path, log_path, log_path)
            )
        log_name_list = []
        log_files = (
            SshClient(self.stdio).run(ssh_helper, get_oblog)
            if self.is_ssh
            else LocalClient(self.stdio).run(get_oblog)
        )
        if log_files:
            log_name_list = get_logfile_name_list(
                self.is_ssh,
                ssh_helper,
                self.from_time_str,
                self.to_time_str,
            ...
            self.stdio.error(
                "Unable to find the log file. Please provide the correct --ob_install_dir, the default is [/home/admin/oceanbase]"
            )
        return log_name_list
    ...
    def __pharse_log_file(
        self, ssh_helper, node, log_name, gather_path, local_store_dir
    ):
        """
        ...
        if self.grep_args is not None:
            ...
            (
                ...
                if self.is_ssh
                else LocalClient(self.stdio).run(grep_cmd)
            )
            log_full_path = "{gather_path}/{log_name}".format(
                log_name=log_name, gather_path=gather_path
            )
            download_file(
                self.is_ssh, ssh_helper, log_full_path, local_store_path, self.stdio
            )
        else:
            real_time_logs = [
                "observer.log",
            ...
            if log_name in real_time_logs:
                cp_cmd = "cp {log_dir}/{log_name} {gather_path}/{log_name} ".format(
                    gather_path=gather_path, log_name=log_name, log_dir=log_path
                )
                self.stdio.verbose("copy files, run cmd = [{0}]".format(cp_cmd))
                (
                    SshClient(self.stdio).run(ssh_helper, cp_cmd)
                    if self.is_ssh
                    else LocalClient(self.stdio).run(cp_cmd)
                )
                log_full_path = "{gather_path}/{log_name}".format(
                    log_name=log_name, gather_path=gather_path
                )
                ...
                log_full_path = "{log_dir}/{log_name}".format(
                    log_name=log_name, log_dir=log_path
                )
                download_file(
                    self.is_ssh, ssh_helper, log_full_path, local_store_path, self.stdio
                )
    ...
    def __pharse_offline_log_file(self, ssh_helper, log_name, local_store_dir):
        ...
        if self.grep_args is not None:
            ...
            download_file(
                self.is_ssh, ssh_helper, log_name, local_store_path, self.stdio
            )
                    ...
                    if len(ret_code) > 1:
                        ...
                        if error_dict.get(ret_code) is None:
                            error_dict[ret_code] = {
                                ...
                                "trace_id_list": (
                                    {trace_id} if len(trace_id) > 0 else {}
                                ),
                            }
                        else:
                            count = error_dict[ret_code]["count"] + 1
                            first_found_time = (
                                error_dict[ret_code]["first_found_time"]
                                if error_dict[ret_code]["first_found_time"] < line_time
                                else line_time
                            )
                            last_found_time = (
                                error_dict[ret_code]["last_found_time"]
                                if error_dict[ret_code]["last_found_time"] > line_time
                                else line_time
                            )
    ...
    def __get_overall_summary(node_summary_tuples, is_files=False):
        ...
        field_names_details.extend(
            ["Cause", "Solution", "First Found Time", "Last Found Time", "Trace_IDS"]
        )
