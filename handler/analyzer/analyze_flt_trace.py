...
class AnalyzeFltTraceHandler(object):
    ...
    def init_option(self):
        ...
        if store_dir_option is not None:
            if not os.path.exists(os.path.abspath(store_dir_option)):
                self.stdio.warn(
                    "Warning: args --store_dir [{0}] incorrect: No such directory, Now create it".format(
    ...
    def handle(self):
        ...
        def handle_from_node(node):
            resp, node_file_s = self.__handle_from_node(
                node, old_files, local_store_parent_dir
            )
        ...
        with ProcessPoolExecutor(
            max(min(self.workers, len(node_files)), 1)
        ) as executor:
            future_to_url = [
                executor.submit(self.parse_file, file) for file in node_files
            ]
    ...
    def __handle_from_node(self, node, old_files, local_store_parent_dir):
        ...
        if not ssh_failed:
            ...
            if self.is_ssh:
                self.__get_online_log_file(
                    ssh, node, gather_dir_full_path, local_store_dir
                )
            else:
            ...
            delete_file(
                ...
                os.path.join(
                    ...
                    str(node.get("host_type")) + "-" + str(self.flt_trace_id),
                ),
                self.stdio,
            )
            ssh.ssh_close()
    ...
    def __get_online_log_file(self, ssh_helper, node, gather_path, local_store_dir):
        ...
        local_store_path = "{0}/{1}".format(
            local_store_dir, str(node.get("host_type")) + "-" + str(self.flt_trace_id)
        )
        ...
        log_full_path = "{gather_path}/{log_name}".format(
            log_name=self.flt_trace_id, gather_path=gather_path
        )
    ...
    def __get_offline_log_file(self, ssh_helper, log_full_path, local_store_dir):
        ...
        if self.flt_trace_id is not None and (len(log_name_list) > 0):
            ...
            download_file(
                False, ssh_helper, log_full_path, local_store_path, self.stdio
            )
        ...
        with open(file, "r", encoding="utf-8") as f:
            while True:
                ...
                if line:
                    ...
                    self.stdio.verbose(
                        "file:{} trace:{} total:{}".format(file, trace, counter)
                    )
                    break
        return li
