...
class Node:
    ...
    def name(self):
        return (
            self.tree_info + self.value["trace_data"]["name"] if self.value else "TRACE"
        )
    ...
    def elapsed_time(self):
        if self.value:
            return TimeUtils.trans_time(
                self.value["trace_data"]["end_ts"]
                - self.value["trace_data"]["start_ts"]
            )
        return "-"
    ...
    def _update_value(self, value):
        if self.value:
            ...
            self.value["trace_data"]["start_ts"] = max(
                self.value["trace_data"]["start_ts"], value["trace_data"]["start_ts"]
            )
            self.value["trace_data"]["end_ts"] = max(
                self.value["trace_data"]["end_ts"], value["trace_data"]["end_ts"]
            )
        else:
            self.value = value
...
class TreeMeta:
    ...
    def format(self, *args):
        ...
        return "| %s|" % "| ".join(
            format_len(elem) for elem in self.fmt_elements
        ).format(*args)
    ...
    def __str__(self):
        return "TreeMeta: counter {} max_name_len {}".format(
            self.counter, self.max_name
        )
    ...
    def record_meta(self, root_node, max_recursion, order_by="start_ts"):
        ...
        def recurse(node: Node, parent_info):
            ...
            if parent_info:
                node.tree_info = (
                    parent_info[:-1]
                    .replace(node_chars[0:2], child_chars[0:2])
                    .replace(node_chars[2:4], child_chars[2:4])
                    + parent_info[-1]
                )
            else:
                node.tree_info = ""
            ...
            ordered_list = sorted(
                node.c_nodes.values(), key=lambda x: x.value["trace_data"][order_by]
            )
