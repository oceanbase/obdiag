    ...
    def execute(self):
        try:
            ...
            if NetUtils.network_connectivity(remote_server) is False:
                self.stdio.warn(
                    "[update] network connectivity failed. Please check your network connection."
                )
                return
            NetUtils.download_file(
                remote_version_file_name, os.path.expanduser(local_version_file_name)
            )
            with open(local_version_file_name, "r") as file:
                remote_data = yaml.safe_load(file)
            if remote_data.get("obdiag_version") is None:
                self.stdio.warn(
                    "obdiag_version is None. Do not perform the upgrade process."
                )
                return
            else:
            ...
            if remote_data.get("remote_tar_sha") is None:
                self.stdio.warn(
                    "remote_tar_sha is None. Do not perform the upgrade process."
                )
                return
            else:
            ...
            if force is False:
                if os.path.exists(os.path.expanduser(local_update_log_file_name)):
                    with open(
                        os.path.expanduser(local_update_log_file_name), "r"
                    ) as file:
                        local_data = yaml.safe_load(file)
                    if (
                        local_data.get("remote_tar_sha") is not None
                        and local_data.get("remote_tar_sha") == self.remote_tar_sha
                    ):
                        self.stdio.warn(
                            "[update] remote_tar_sha as local_tar_sha. No need to update."
                        )
                        return
                    # get data_update_time
                    if (
                        local_data.get("data_update_time") is not None
                        and time.time() - local_data["data_update_time"] < 3600 * 24 * 7
                    ):
            ...
            # check_sha
            self.local_update_file_sha = FileUtil.calculate_sha256(
                local_update_file_name
            )
            ...
            if os.path.exists(os.path.expanduser("~/.obdiag/check")):
                os.rename(
                    os.path.expanduser("~/.obdiag/check"),
                    os.path.expanduser("~/.obdiag/check.d"),
                )
            ## gather
            ...
            if os.path.exists(os.path.expanduser("~/.obdiag/gather")):
                os.rename(
                    os.path.expanduser("~/.obdiag/gather"),
                    os.path.expanduser("~/.obdiag/gather.d"),
                )
            ...
            if os.path.exists(os.path.expanduser("~/.obdiag/rca")):
                os.rename(
                    os.path.expanduser("~/.obdiag/rca"),
                    os.path.expanduser("~/.obdiag/rca.d"),
                )
            # decompression remote files
            FileUtil.extract_tar(
                os.path.expanduser(local_update_file_name),
                os.path.expanduser("~/.obdiag"),
            )
            # update data save
