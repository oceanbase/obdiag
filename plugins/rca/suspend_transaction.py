...
class SuspendTransactionScene(RcaScene):
    ...
    def execute(self):
        ...
        sql = "select count(1) as suspend_transaction_count from oceanbase.GV$OB_TRANSACTION_PARTICIPANTS t,oceanbase.__all_tenant a where t.tenant_id=a.tenant_id and t.LAST_REQUEST_TIME < date_sub(now(), INTERVAL 600 SECOND) and a.tenant_name='{0}' and t.ROLE = 'LEADER' and t.ACTION <> 'START';".format(
            self.tenant_name
        )
        ...
        sql = "SELECT t.tenant_name,m.svr_ip,m.svr_port,round(m.active_span / 1024 / 1024 / 1024, 5) active_gb,round(m.freeze_trigger / 1024 / 1024 / 1024, 5) trigger_gb,round(m.memstore_used / 1024 / 1024 / 1024, 5) used_gb,round(m.memstore_limit / 1024 / 1024 / 1024, 5) limit_gb,round(m.memstore_used/m.memstore_limit,5) as memstore_use_ratio,m.freeze_cnt freeze_count FROM oceanbase.__all_virtual_tenant_memstore_info m INNER JOIN oceanbase.__all_tenant t ON t.tenant_id = m.tenant_id WHERE t.tenant_name = '{0}';".format(
            self.tenant_name
        )
        ...
        for memstore_use_data_item in memstore_use_data:
            ...
            self.record.add_suggest(
                "{0} in this {1} exist memstore_use full, memstore_use_ratio is {2},maybe you can try expanding the tenant's memory,If expanding the tenant's memory doesn't work, you can try restarting the observer:{3}".format(
                    tenant_name, svr_ip, memstore_use_ratio, svr_ip
                )
            )
            return
        # find cluster disk
        ...
        sql = "select t.tenant_name as tenant_name,a.svr_ip as svr_ip,  round(a.LOG_DISK_IN_USE/a.LOG_DISK_SIZE,2)as clog_disk_ratio from oceanbase.gv$ob_units a ,oceanbase.__all_tenant t where a.tenant_id=t.tenant_id  and t.tenant_name='{0}';".format(
            self.tenant_name
        )
        ...
        for clog_reply_data_item in clog_reply_data:
            ...
            self.record.add_suggest(
                "{0}'s {1} on this {2} exist replay_delay,replay_delay_minutes is {3},you can try expanding tenant's specifications,if expanding the tenant's specifications doesn't work,please contact the  OcenBase community".format(
                    tenant_name, ls_id, svr_ip, replay_delay_minutes
                )
            )
            return
        # other scene
