...
class AutoSplitErrorTable(TaskBase):
    ...
    def _check_table_split_condition(self, table_info):
        ...
        try:
                ...
                for i, tablet_info in enumerate(oversized_tablets):
                    if i >= max_warnings:
                        ...
                        self.report.add_warning(
                            "Table {0}.{1} has {2} tablets with size >= auto_part_size {3}MB, "
                            "showing first {4} tablets, {5} more tablets not shown".format(
                                database_name, table_name, total_oversized_count, auto_part_size, max_warnings, remaining_count
                            )
                        )
                        break
                    
                    self.report.add_warning(
                        "Table {0}.{1} has tablet {2} with size {3}MB >= auto_part_size {4}MB, "
                        "should trigger auto split but hasn't".format(
                            database_name,
                            table_name,
                            tablet_info["tablet_id"],
                            tablet_info["required_size"],
                            auto_part_size
                        )
                    )
            else:
            ...
            self.stdio.error("Failed to check table {0}.{1}: {2}".format(
    ...
    def _check_tablet_sizes(self, tablet_ids, auto_part_size):
        ...
        AND required_size >= {1}
        '''.format(tablet_id_list, auto_part_size)
        ...
        try:
            ...
            for row in result:
                tablet_info = {
                    "tablet_id": row.get("tablet_id") or row.get("TABLET_ID"),
                    "required_size": row.get("required_size") or row.get("REQUIRED_SIZE")
                }
