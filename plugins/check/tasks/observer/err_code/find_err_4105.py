...
"""
@time: 2024/12/29
@file: find_err_4105.py
@desc: Check whether Error 4105 is reported when enable_sql_audit is set to True
"""

from src.handler.check.check_task import TaskBase


class FindErr4105Task(TaskBase):
    ...
    def execute(self):
        try:
            ...
            # Check for error 4105
            sql = 'select count(0) as err_count from oceanbase.GV$OB_SQL_AUDIT where RET_CODE="-4105"'
            try:
                result = self.ob_connector.execute_sql_return_cursor_dictionary(sql).fetchall()
                if result and len(result) > 0:
                    err_count = result[0].get('err_count', 0)
                    if err_count > 0:
                        self.report.add_critical("number of sql_error_4105 is {0}".format(err_count))
            except Exception as e:
                self.report.add_fail("Failed to check error 4105: {0}".format(e))

        except Exception as e:
            self.report.add_fail("Execution error: {0}".format(e))

    def get_task_info(self):
        return {"name": "find_err_4105", "info": "Check whether Error 4105 is reported when enable_sql_audit is set to True."}


find_err_4105 = FindErr4105Task()
