...
"""
@time: 2024/12/29
@file: sysbench_test_cluster_log_disk_size.py
@desc: Check cluster log_disk_size parameters for sysbench
"""

from src.handler.check.check_task import TaskBase


class SysbenchTestClusterLogDiskSizeTask(TaskBase):
        ...
        try:
            ...
            for node in self.observer_nodes:
                ...
                try:
                    ...
                    self.stdio.error("Failed to check log disk size on {0}: {1}".format(node_name, e))

        except Exception as e:
            self.report.add_fail("Execution error: {0}".format(e))

    def get_task_info(self):
        return {"name": "sysbench_test_cluster_log_disk_size", "info": "Check cluster log_disk_size parameters for sysbench."}


sysbench_test_cluster_log_disk_size = SysbenchTestClusterLogDiskSizeTask()
