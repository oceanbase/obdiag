                    ...
                    if timeout_seconds is not None:
                        ...
                        if timeout_seconds > 86400:
                            self.report.add_warning(
                                ...
                                "Consider reducing the timeout value to prevent thread exhaustion issues.".format(
                                    value, timeout_seconds
                                )
                            )
                            self.stdio.warn("ob_query_timeout exceeds 24 hours: {0} microseconds ({1} seconds)".format(
                                value, timeout_seconds
                            ))
                        else:
                            self.stdio.verbose("ob_query_timeout is within acceptable range: {0} microseconds ({1} seconds)".format(
                                value, timeout_seconds
                            ))
                    else:
