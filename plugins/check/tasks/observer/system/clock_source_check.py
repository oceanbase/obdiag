...
class ClockSourceCheck(TaskBase):
    ...
    def execute(self):
        try:
            ...
            if non_compliant_nodes:
                ...
                return self.report.add_warning(
                    f"Found nodes with inconsistent clock sources (expected: {list(most_common_config)}): {nodes_str}"
                )            
    ...
    def get_task_info(self):
        return {
            ...
            "info": "It is recommended to add inspection items to check whether the configuration file server IP of the ob node clock source is consistent.issue #781"
        }
