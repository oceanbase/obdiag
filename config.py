        ...
        for section, values in default_config.items():
            if isinstance(values, dict):
                for key, default_value in values.items():
                    if (
                        section not in combined_config
                        or key not in combined_config[section]
                    ):
...
class ConfigManager(Manager):
    ...
    def __init__(self, config_file=None, stdio=None):
        default_config_path = os.path.join(
            os.path.expanduser("~"), ".obdiag", "config.yml"
        )

        if config_file is None or not os.path.exists(config_file):
            config_file = default_config_path
            pathlib.Path(os.path.dirname(default_config_path)).mkdir(
    ...
    def get_ob_cluster_config(self):
        ...
        def create_ob_cluster_node(node_config, global_config):
            return {
                ...
                "ssh_username": node_config.get(
                    "ssh_username", global_config.get("ssh_username")
                ),
                "ssh_password": node_config.get(
                    "ssh_password", global_config.get("ssh_password")
                ),
                "ssh_port": node_config.get(
                    "ssh_port", global_config.get("ssh_port", 22)
                ),
                "home_path": node_config.get(
                    "home_path", global_config.get("home_path", "/root/observer")
                ),
                "data_dir": node_config.get(
                    "data_dir", global_config.get("data_dir", "/root/observer/store")
                ),
                "redo_dir": node_config.get(
                    "redo_dir", global_config.get("redo_dir", "/root/observer/store")
                ),
                "ssh_key_file": node_config.get(
                    "ssh_key_file", global_config.get("ssh_key_file", "")
                ),
                "ssh_type": node_config.get(
                    "ssh_type", global_config.get("ssh_type", "remote")
                ),
                "container_name": node_config.get(
        ...
        ob_cluster_nodes = [
            create_ob_cluster_node(node, global_config) for node in nodes
        ]
    ...
    def get_obproxy_config(self):
        ...
        def create_ob_proxy_node(node_config, global_config):
            return {
                ...
                "home_path": node_config.get(
                    "home_path", global_config.get("home_path", "/root/obproxy")
                ),
                "ssh_key_file": node_config.get(
                    "ssh_key_file", global_config.get("ssh_key_file", "")
                ),
                "ssh_type": node_config.get(
                    "ssh_type", global_config.get("ssh_type", "ssh")
                ),
                "container_name": node_config.get(
                    "container_name", global_config.get("container_name")
                ),
