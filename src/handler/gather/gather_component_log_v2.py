...
class BaseLogGatherer(ABC):
    """Abstract base class for log gatherers of different components"""

    def __init__(self, stdio=None):
        self.stdio = stdio

    @abstractmethod
    def get_log_path(self, node, config):
        """
        Get the log path for the component
        :param node: node configuration dict
        :param config: gather configuration dict
        :return: log path string
        """
        pass

    @abstractmethod
    def filter_log_files(self, log_files, from_time_str, to_time_str, log_dir, ssh_client):
        """
        Filter log files based on time range
        :param log_files: raw log file list (string with newlines)
        :param from_time_str: start time string (format: "YYYY-MM-DD HH:MM:SS")
        :param to_time_str: end time string (format: "YYYY-MM-DD HH:MM:SS")
        :param log_dir: log directory path
        :param ssh_client: SSH client instance
        :return: list of filtered log file names
        """
        pass
    ...
    def get_log_scope(self):
        """
        Get log scope configuration for the component
        :return: dict of log scope configuration
        """
        pass

    def get_tmp_dir_name(self, target, ssh_client, from_time_str, to_time_str, node=None):
        """
        Get temporary directory name for log gathering
        :param target: component target name
        :param ssh_client: SSH client instance
        :param from_time_str: start time string
        :param to_time_str: end time string
        :param node: node configuration dict (optional, for special cases like observer PID)
        :return: temporary directory name string
        """
        from_datetime_timestamp = TimeUtils.timestamp_to_filename_time(TimeUtils.datetime_to_timestamp(from_time_str))
        to_datetime_timestamp = TimeUtils.timestamp_to_filename_time(TimeUtils.datetime_to_timestamp(to_time_str))
        tmp_dir = "{0}_log_{1}_{2}_{3}_{4}".format(target, ssh_client.get_name().replace(":", "_"), from_datetime_timestamp, to_datetime_timestamp, str(uuid.uuid4())[:6])
        return tmp_dir

    def _filter_log_files_by_timestamp(self, log_files, from_time_str, to_time_str, log_dir, ssh_client):
        """
        Common method to filter log files by timestamp in filename
        Used by observer and obproxy components
        """
        ...
        try:
            ...
            self.stdio.error("gather_log_on_node {0} parse time failed: {1}".format(ssh_client.get_ip(), str(e)))
            ...
            try:
                ...
                if timestamp_match:
                    ...
                    try:
                        ...
                        self.stdio.verbose("node: {0}, file_name: {1}, extracted timestamp: {2}, file_time: {3}".format(ssh_client.get_name(), file_name, timestamp_str, file_time_dt.strftime("%Y-%m-%d %H:%M:%S.%f")))

                        # Check if file time is within the query range
                        if from_time_dt <= file_time_dt <= to_time_dt:
                            log_name_list.append(file_name)
                            self.stdio.verbose("node: {0}, file {1} timestamp {2} is in range [{3}, {4}], include it".format(ssh_client.get_name(), file_name, file_time_dt.strftime("%Y-%m-%d %H:%M:%S"), from_time_str, to_time_str))
                        else:
                            self.stdio.verbose("node: {0}, file {1} timestamp {2} is out of range [{3}, {4}], exclude it".format(ssh_client.get_name(), file_name, file_time_dt.strftime("%Y-%m-%d %H:%M:%S"), from_time_str, to_time_str))
                    except (ValueError, IndexError) as e:
                        self.stdio.warn("node: {0}, file {1} has invalid timestamp format, skip: {2}".format(ssh_client.get_name(), file_name, str(e)))
                        continue
                else:
                    ...
                    if file_name.endswith(".log") or file_name.endswith(".wf"):
                        log_name_list.append(file_name)
                        self.stdio.verbose("node: {0}, file {1} has no timestamp, is current log file, include it".format(ssh_client.get_name(), file_name))
                    else:
                        self.stdio.verbose("node: {0}, file {1} has no timestamp and is not a .log or .wf file, skip".format(ssh_client.get_name(), file_name))

            except Exception as e:
                self.stdio.error("gather_log_on_node {0} get log file: {2} name failed, Skip it: {1}".format(ssh_client.get_ip(), str(e), file_name))
                self.stdio.verbose(traceback.format_exc())
                continue

        if len(log_name_list) > 0:
            self.stdio.verbose("Find the qualified log file {0} on Server [{1}], wait for the next step".format(log_name_list, ssh_client.get_ip()))
        else:
            self.stdio.warn("No found the qualified log file on Server [{0}]".format(ssh_client.get_name()))
        return log_name_list


class ObserverLogGatherer(BaseLogGatherer):
    """Log gatherer for observer component"""

    def get_log_path(self, node, config):
        return os.path.join(node.get("home_path"), "log")

    def filter_log_files(self, log_files, from_time_str, to_time_str, log_dir, ssh_client):
        return self._filter_log_files_by_timestamp(log_files, from_time_str, to_time_str, log_dir, ssh_client)

    def get_log_scope(self):
        return {"observer": {"key": "*observer*"}, "rootservice": {"key": "*rootservice*"}, "election": {"key": "*election*"}}

    def get_tmp_dir_name(self, target, ssh_client, from_time_str, to_time_str, node=None):
        tmp_dir = super().get_tmp_dir_name(target, ssh_client, from_time_str, to_time_str, node)
        # For observer, add PID if local mode
        if node and ssh_client.get_name() == "local":
            pid = self._get_observer_pid(node)
            if pid:
                tmp_dir = "{0}_pid_{1}".format(tmp_dir, pid)
        return tmp_dir

    def _get_observer_pid(self, node):
...
class ObproxyLogGatherer(BaseLogGatherer):
    """Log gatherer for obproxy component"""
...
class OmsLogGatherer(BaseLogGatherer):
    """Log gatherer for oms component"""

    def get_log_path(self, node, config):
        oms_component_id = config.get("oms_component_id")
        if oms_component_id is None:
        ...
        return os.path.join(node.get("run_path"), oms_component_id, "logs")
    ...
    def get_log_scope(self):
        return {"connector": {"key": "*connector.*"}, "error": {"key": "error"}, "trace.log": {"key": "trace.log"}, "metrics": {"key": "metrics*"}}


class OmsCdcLogGatherer(BaseLogGatherer):
    """Log gatherer for oms_cdc component"""
    ...
    def get_log_path(self, node, config):
        ...
        obcdc_id = None
        match = re.search(r"\d+\.\d+\.\d+\.\d+-(\d+)", oms_component_id)
        if match:
            number = match.group(1)
            obcdc_id = number
        else:
            self.stdio.error("can not get obcdc_id by component_id. please check component_id.")
        return os.path.join(node.get("store_path"), "store" + obcdc_id, "log")

    def filter_log_files(self, log_files, from_time_str, to_time_str, log_dir, ssh_client):
        # oms_cdc not support get log file name list, return all 'libobcdc.log*' log file name list
    ...
    def get_log_scope(self):
        return {"libobcdc": {"key": "*libobcdc.log*"}, "removed_log_files": {"key": "removed_log_files"}, "store": {"key": "store.log"}}


class GatherLogOnNode_v2:
    """Node-level log gathering handler using LogGatherer interface"""

    def __init__(self, context, node, config, log_gatherer):
        self.context = context
        self.ssh_client = None
        self.stdio = context.stdio
        self.config = config
        self.node = node
        self.log_gatherer = log_gatherer
        ...
        self.log_path = self.log_gatherer.get_log_path(node, config)
    ...
    def handle(self):
        ...
        tmp_dir = self.log_gatherer.get_tmp_dir_name(self.target, self.ssh_client, self.from_time_str, self.to_time_str, self.node)
    ...
    def __grep_log_to_tmp(self, logs_name, tmp_log_dir):
        """Copy or grep log files to temporary directory"""
        grep_cmd = ""
        if self.grep_option:
    ...
    def __find_logs_name(self):
        """Find log files based on scope and filter by time range"""
        try:
            logs_scope = ""
            ...
            if logs_name is not None and len(logs_name) != 0:
                # Use log_gatherer to filter log files
                log_name_list = self.log_gatherer.filter_log_files(logs_name, self.from_time_str, self.to_time_str, self.log_path, self.ssh_client)
                return log_name_list
            else:
...
class GatherComponentLogHandler_v2(BaseShellHandler):
    """Refactored log gathering handler using abstract interface pattern"""
    ...
    def init(self, context, *args, **kwargs):
        try:
            ...
            # Create log gatherer instance based on target
            self.log_gatherer = self.__create_log_gatherer()
            self.log_gatherer.stdio = self.stdio
            ...
            self.stdio.error("init GatherComponentLogHandler_v2 failed, error: {0}".format(str(e)))
            self.result = ObdiagResult(ObdiagResult.INPUT_ERROR_CODE, error_data="init GatherComponentLogHandler_v2 failed, error: {0}".format(str(e)))

    def __create_temp_log_gatherer(self):
        """Create temporary log gatherer instance for validation (before full init)"""
        if self.target == 'observer':
            return ObserverLogGatherer(self.stdio)
        elif self.target == 'obproxy':
            return ObproxyLogGatherer(self.stdio)
        elif self.target == 'oms':
            return OmsLogGatherer(self.stdio)
        elif self.target == 'oms_cdc':
            return OmsCdcLogGatherer(self.stdio)
        else:
            raise Exception("Unknown target: {0}".format(self.target))

    def __create_log_gatherer(self):
        """Create appropriate log gatherer instance based on target"""
        return self.__create_temp_log_gatherer()
    ...
    def __check_option(self):
        ...
        # check scope - create temporary gatherer to validate scope
        temp_gatherer = self.__create_temp_log_gatherer()
        if self.scope is None or self.scope == "" or self.scope == "all":
            # Will be set in handle() method using log_gatherer
            pass
        else:
            ...
            default_scope = temp_gatherer.get_log_scope()
            if self.scope not in default_scope:
                raise Exception("scope option can only be {0},the {1} just support {2}".format(self.scope, self.target, default_scope))
            self.scope = {self.scope: default_scope[self.scope]}
    ...
    def handle(self):
        try:
            ...
            if self.scope is None or self.scope == "" or self.scope == "all":
                self.scope = self.log_gatherer.get_log_scope()
            else:
                if isinstance(self.scope, str):
                    self.scope = self.scope.strip()
                else:
                    raise Exception("scope option can only be string")
                default_scope = self.log_gatherer.get_log_scope()
                if self.scope not in default_scope:
                    raise Exception("scope option can only be {0},the {1} just support {2}".format(self.scope, self.target, default_scope))
                self.scope = {self.scope: default_scope[self.scope]}

            # Update config dict with scope
            self.gather_log_conf_dict["scope"] = self.scope
            ...
            try:
                for node in self.nodes:
                    ...
                    # use Process must delete ssh_client, and GatherLogOnNode_v2 will rebuild it.
                    if "ssh_client" in node or "ssher" in node:
                        ...
                        tasks.append(GatherLogOnNode_v2(new_context, clear_node, self.gather_log_conf_dict, self.log_gatherer))
                    else:
                        tasks.append(GatherLogOnNode_v2(new_context, node, self.gather_log_conf_dict, self.log_gatherer))
                file_queue = []
