            ...
            if len(tenant_memory_info_dict) == 0:
                ...
                for tenant_id in tenant_memory_info_dict:
                    ...
                    for ctx_name in ctx_memory_info_dict:
                        ...
                        fig_combined.add_trace(go.Scatter(x=x_lines, y=ctx_hold_lines, mode='lines', name='{0}'.format(ctx_name)), row=2, col=1)
                    if len(mod_memory_info_dict) > 10:
                        ...
                        for key in top_10_keys:
                            ...
                            fig_combined.add_trace(go.Scatter(x=x_lines, y=mod_hold_lines, mode='lines', name='{0}'.format(key)), row=3, col=1)
                    else:
                        for key in mod_memory_info_dict:
                            if '-SUMMARY' not in key:
                                mod_hold_lines = [round(mod_memory_info_dict[key][t] / 1024 / 1024) for t in sorted(mod_memory_info_dict[key].keys())]
                                fig_combined.add_trace(go.Scatter(x=x_lines, y=mod_hold_lines, mode='lines', name='{0}'.format(key)), row=3, col=1)
                    fig_combined.update_xaxes(tickvals=x_vals, ticktext=[str(x) for x in x_vals], row=1, col=1)
                    fig_combined.update_xaxes(tickvals=x_vals, ticktext=[str(x) for x in x_vals], row=2, col=1)
                    fig_combined.update_xaxes(tickvals=x_vals, ticktext=[str(x) for x in x_vals], row=3, col=1)
                ...
                index_html = '''
                    ...
                    <body>
                        ...
                        <div class="tenant-list">
                            ...
                            <p><a href="TOP15_tenant_hold_memory.html">TOP 15 租户 hold 内存曲线图</a></p>
                        </div>
                    </body>
                    </html>
                    '''.format(
