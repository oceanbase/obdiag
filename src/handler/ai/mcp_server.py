...
class MCPServer:
    ...
    def __init__(self, config_path: Optional[str] = None, stdio=None, context=None):
        ...
        self._db_connectors: Dict[str, OBConnector] = {}  # Store database connections by connection_id
    ...
    def _register_tools(self) -> List[Dict[str, Any]]:
        ...
        return [
            ...
            {
                "name": "db_connect",
                "description": "Connect to OceanBase database. Returns a connection_id that can be used for subsequent queries.",
                "inputSchema": {
                    "type": "object",
                    "properties": {
                        "connection_id": {"type": "string", "description": "Unique identifier for this connection (optional, auto-generated if not provided)"},
                        "ip": {"type": "string", "description": "Database host IP address"},
                        "port": {"type": "integer", "description": "Database port"},
                        "username": {"type": "string", "description": "Database username"},
                        "password": {"type": "string", "description": "Database password"},
                        "database": {"type": "string", "description": "Database name (optional)"},
                        "timeout": {"type": "integer", "description": "Connection timeout in seconds, default 30", "default": 30},
                    },
            ...
            {
                "name": "db_query",
                "description": "Execute SQL query on a connected database. Must call db_connect first to establish a connection.",
                "inputSchema": {
                    "type": "object",
                    "properties": {
    ...
    def _call_tool(self, tool_name: str, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        elif tool_name == "db_connect":
            return self._db_connect(arguments)
        elif tool_name == "db_query":
            return self._db_query(arguments)
        elif tool_name == "db_close":
            return self._db_close(arguments)
    ...
    def _db_connect(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not all([ip, port, username, password]):
                return {
                    "content": [{"type": "text", "text": "Missing required parameters: ip, port, username, password"}],
                    "isError": True,
                }

            # Generate connection_id if not provided
            if not connection_id:
                connection_id = f"{username}@{ip}:{port}"
                if database:
                    connection_id += f"/{database}"

            # Create a minimal context if not provided
            if not self.context:
                # Create a simple context-like object with stdio
                class SimpleContext:
                    def __init__(self, stdio):
                        self.stdio = stdio

                context = SimpleContext(self.stdio)
            else:
                context = self.context

            if self.stdio:
                self.stdio.verbose(f"Connecting to database {ip}:{port} as {username}...")
            ...
            # Test connection
            test_result = connector.execute_sql("SELECT 1")
            if test_result is None:
                return {
                    "content": [{"type": "text", "text": "Connection test failed"}],
                    "isError": True,
                }

            # Store connection
            self._db_connectors[connection_id] = connector

            success_msg = f"Successfully connected to database {ip}:{port}"
            if database:
                success_msg += f" (database: {database})"
    ...
    def _db_query(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not connection_id or not sql:
                return {
                    "content": [{"type": "text", "text": "Missing required parameters: connection_id, sql"}],
                    "isError": True,
                }

            if connection_id not in self._db_connectors:
                return {
                    "content": [
                        {
                            "type": "text",
                            "text": f"Connection {connection_id} not found. Please call db_connect first.",
                        }
                    ],
                    "isError": True,
                }

            connector = self._db_connectors[connection_id]

            if self.stdio:
                self.stdio.verbose(f"Executing SQL query on {connection_id}: {sql[:100]}...")
            ...
            if self.stdio:
                self.stdio.verbose(f"Query returned {len(results)} rows")
    ...
    def _db_close(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not connection_id:
                return {
                    "content": [{"type": "text", "text": "Missing required parameter: connection_id"}],
                    "isError": True,
                }

            if connection_id not in self._db_connectors:
                return {
                    "content": [{"type": "text", "text": f"Connection {connection_id} not found"}],
                    "isError": True,
                }
            ...
            del self._db_connectors[connection_id]

            success_msg = f"Connection {connection_id} closed successfully"
            if self.stdio:
                self.stdio.verbose(success_msg)
            ...
            error_msg = f"Failed to close connection: {str(e)}"
            if self.stdio:
                self.stdio.verbose(error_msg)
            return {"content": [{"type": "text", "text": error_msg}], "isError": True}
    ...
    def _file_write(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not file_path or content is None:
                return {
                    "content": [{"type": "text", "text": "Missing required parameters: file_path, content"}],
                    "isError": True,
                }
