...
class ObdiagOriginCommand(BaseCommand):
    ...
    def do_command(self):
        ...
        if self.has_trace:
            ...
            ROOT_IO.print(
                "If you want to view detailed obdiag logs, please run: obdiag display-trace %s"
                % trace_id
            )
        return ret
    ...
    def get_white_ip_list(self):
        ...
        ROOT_IO.warn(
            "Security Risk: the whitelist is empty and anyone can request this program!"
        )
...
class DisplayTraceCommand(ObdiagOriginCommand):
    ...
    def __init__(self):
        super(DisplayTraceCommand, self).__init__(
    ...
    def _do_command(self, obdiag):
        ...
        try:
            ...
            return False
        cmd = 'cd {} && grep -h "\[{}\]" $(ls -tr {}*) | sed "s/\[{}\] //g" '.format(
            log_dir, trace_id, log_dir, trace_id
        )
...
class ObdiagGatherAllCommand(ObdiagOriginCommand):
    ...
    def __init__(self):
        super(ObdiagGatherAllCommand, self).__init__(
        ...
        self.parser.add_option(
            "--scope",
            type="string",
            help="log type constrains, choices=[observer, election, rootservice, all]",
            default="all",
        )
        self.parser.add_option(
            "--grep", action="append", type="string", help="specify keywords constrain"
        )
        self.parser.add_option(
            "--encrypt",
            type="string",
            help="Whether the returned results need to be encrypted, choices=[true, false]",
            default="false",
        )
        self.parser.add_option(
            "--store_dir",
...
class ObdiagGatherLogCommand(ObdiagOriginCommand):
    ...
    def __init__(self):
        super(ObdiagGatherLogCommand, self).__init__(
...
class ObdiagGatherSysStatCommand(ObdiagOriginCommand):
    ...
    def __init__(self):
        super(ObdiagGatherSysStatCommand, self).__init__(
...
class ObdiagGatherPerfCommand(ObdiagOriginCommand):
    ...
    def __init__(self):
        ...
        self.parser.add_option(
            "--scope",
            type="string",
            help="perf type constrains, choices=[sample, flame, pstack, all]",
            default="all",
        )
        self.parser.add_option(
            "-c",
...
class ObdiagGatherAwrCommand(ObdiagOriginCommand):
    ...
    def __init__(self):
        super(ObdiagGatherAwrCommand, self).__init__(
            "awr", "Gather ParalleSQL information"
        )
        self.parser.add_option(
            "--cluster_name", type="string", help="cluster_name from ocp"
        )
        self.parser.add_option(
            "--cluster_id", type="string", help="cluster_id from ocp"
        )
        self.parser.add_option(
            "--from",
            type="string",
...
class ObdiagGatherPlanMonitorCommand(ObdiagOriginCommand):
    ...
    def __init__(self):
        super(ObdiagGatherPlanMonitorCommand, self).__init__(
        ...
        self.parser.add_option(
            "--env",
            type="string",
            help='''env, eg: "{db_connect='-h127.0.0.1 -P2881 -utest@test -p****** -Dtest'}"''',
        )
        self.parser.add_option(
            "-c",
            type="string",
...
class ObdiagGatherObproxyLogCommand(ObdiagOriginCommand):
    ...
    def __init__(self):
        super(ObdiagGatherObproxyLogCommand, self).__init__(
        ...
        self.parser.add_option(
            "--scope",
            type="string",
            help="log type constrains, choices=[obproxy, obproxy_limit, obproxy_stat, obproxy_digest, obproxy_slow, obproxy_diagnosis, obproxy_error, all]",
            default="all",
        )
        self.parser.add_option(
            "--grep", action="append", type="string", help="specify keywords constrain"
...
class ObdiagGatherSceneRunCommand(ObdiagOriginCommand):
    ...
    def __init__(self):
        ...
        self.parser.add_option(
            "--scene", type="string", help="Specify the scene to be gather"
        )
        self.parser.add_option(
            "--from",
            type="string",
        ...
        self.parser.add_option(
            "--env", type="string", help='env, eg: "{env1=xxx, env2=xxx}"'
        )
        self.parser.add_option(
            "--store_dir",
            type="string",
...
class ObdiagGatherAshReportCommand(ObdiagOriginCommand):
    ...
    def __init__(self):
        ...
        # WAIT_CLASS
        self.parser.add_option(
            "--wait_class", type="string", help="Event types to be sampled."
        )
        self.parser.add_option(
            "--report_type",
            type="string",
            help="Report type, currently only supports text type.",
            default="TEXT",
        )
        self.parser.add_option(
            "--from",
...
class ObdiagAnalyzeLogCommand(ObdiagOriginCommand):
    ...
    def __init__(self):
        super(ObdiagAnalyzeLogCommand, self).__init__(
            "log",
            "Analyze oceanbase log from online observer machines or offline oceanbase log files",
        )
        self.parser.add_option(
            "--from",
            type="string",
        ...
        self.parser.add_option(
            "--log_level",
            type="string",
            help="oceanbase logs greater than or equal to this level will be analyze, choices=[DEBUG, TRACE, INFO, WDIAG, WARN, EDIAG, ERROR]",
        )
        self.parser.add_option(
            "--files", action="append", type="string", help="specify files"
        )
        self.parser.add_option(
            "--store_dir",
            type="string",
...
class ObdiagAnalyzeFltTraceCommand(ObdiagOriginCommand):
    ...
    def __init__(self):
        super(ObdiagAnalyzeFltTraceCommand, self).__init__(
            "flt_trace",
            "Analyze oceanbase trace.log from online observer machines or offline oceanbase trace.log files",
        )
        self.parser.add_option(
            "--flt_trace_id",
            type="string",
            help="flt trace id, . format: xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",
        )
        ...
        self.parser.add_option(
            "--recursion", type="string", help="Maximum number of recursion", default=8
        )
        self.parser.add_option(
            "--output",
            type="string",
...
class ObdiagCheckCommand(ObdiagOriginCommand):
    ...
    def __init__(self):
        ...
        self.parser.add_option(
            "--cases", type="string", help="check observer's cases on package_file"
        )
        self.parser.add_option(
            "--obproxy_cases",
            type="string",
            help="check obproxy's cases on package_file",
        )
        self.parser.add_option(
            "--store_dir",
            type="string",
            help="the dir to store check result, current dir by default.",
            default="./check_report/",
        )
        self.parser.add_option(
            "-c",
            type="string",
...
class ObdiagRCARunCommand(ObdiagOriginCommand):
    ...
    def __init__(self):
        ...
        self.parser.add_option(
            "--scene", type="string", help="rca scene name. The argument is required."
        )
        self.parser.add_option(
            "--store_dir",
            type="string",
            help="the dir to store rca result, current dir by default.",
            default="./rca/",
        )
        self.parser.add_option(
            "--input_parameters", type="string", help="input parameters of scene"
        )
        self.parser.add_option(
            "-c",
            type="string",
...
class ObdiagUpdateCommand(ObdiagOriginCommand):
    ...
    def __init__(self):
        ...
        self.parser.add_option(
            "--file",
            type="string",
            help="obdiag update cheat file path. Please note that you need to ensure the reliability of the files on your own.",
        )
        self.parser.add_option(
            "--force",
            action="store_true",
...
class ObdiagGatherCommand(MajorCommand):
    ...
    def __init__(self):
        super(ObdiagGatherCommand, self).__init__(
            "gather", "Gather oceanbase diagnostic info"
        )
...
class ObdiagGatherSceneCommand(MajorCommand):
    ...
    def __init__(self):
        super(ObdiagGatherSceneCommand, self).__init__(
...
class ObdiagAnalyzeCommand(MajorCommand):
    ...
    def __init__(self):
        super(ObdiagAnalyzeCommand, self).__init__(
            "analyze", "Analyze oceanbase diagnostic info"
        )
